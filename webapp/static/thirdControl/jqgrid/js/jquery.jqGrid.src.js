...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {loadTotal:"loadTotal",page:"page",rows:"rows", sort: "sidx",order: "sord", search:"_search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
                ...
                //lst 增加是否加载总条数的参数
                if(pN.loadTotal !== null) {prm[pN.loadTotal]= ts.p.loadTotal;}
		...
		setPager = function (pgid, tp){
			...
			if(dir==="ltr") { pgl += str; }
			pgl += "</tr></tbody></table>";
            if(ts.p.viewrecords===true && ts.p.loadTotalButton===true) {
                if(ts.p.loadTotal===true){
                    $("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<div><span id='"+pgid+"_records' dir='"+dir+"' style='text-align:"+ts.p.recordpos+"' class='ui-paging-info'></span> <span><button type='button' class='btn btn-xs btn-default' id='"+pgid+"_total' > <span>关闭</span> </button></span> </div>");
                }else{
                    $("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<span><button type='button' class='btn btn-xs btn-default' id='"+pgid+"_total' > <span>分页信息</span> </button></span> ");
                }
            }
            if(ts.p.viewrecords===false && ts.p.loadTotalButton===true) {
                if(ts.p.loadTotal===true){
                    $("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<span><button type='button' class='btn btn-xs btn-default' id='"+pgid+"_total' > <span>关闭</span> </button></span> ");
                }else{
                    $("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<span><button type='button' class='btn btn-xs btn-default' id='"+pgid+"_total' > <span>分页信息</span> </button></span> ");
                }
            }
            if(ts.p.viewrecords===true && ts.p.loadTotalButton===false) {
                if(ts.p.loadTotal===true) {
			...
			if(ts.p.loadTotalButton===true) {
				$('#'+pgid+'_total').on('click',function () {
					$("#"+pgid).html("");
					if(ts.p.loadTotal===true){
						ts.p.loadTotal=false;
						ts.p.pagerpos="left";
						ts.p.pgtext="{0}";
					}else{
						ts.p.loadTotal=true;
					...
					//加载总条数时 刷新当前页
					if(ts.p.loadTotal===true){
						$(ts).trigger("reloadGrid");//重新加载JqGrid
					}else{
						//加载页数等数据
