...
$(function () {
    ...
    $("#fwTab,#lqTab,#cqTab,#tdTab").click(function () {
        ...
        if (this.id == "dyhTab") {
            ...
            if (this.id == "fwTab") {
                ...
                tdTableInit();
                if (isLoadGrid("td"))
                    tableReload("td-grid-table", tdUrl, {hhSearch: '',sfsh:'0'}, '', '');
            }
    ...
    $("#dyh_search").click(function () {
        ...
        tableReload("dyh-grid-table", dyhUrl, {hhSearch: hhSearch, bdclx: $bdclx}, dyhColModel, '');
    })
...
//                }
    ...
    $("#save").click(function () {
        ...
        if (bdcdyhs == null || bdcdyhs == '' && ( bppwfids.indexOf(sqlxdm) < 0) && (djlx!=null && djlx!="" && djlx!=800&& djlx!=400)) {
            tipInfo("请选择不动产单元");
            return false;
...
//                        if(bdcdyhs.length>1 || djlx=='800'){
    ...
    $("#match").click(function () {
        ...
        var bdcdyDjIds = getBdcdyDjIds();
        var djlx = $("#djlx").val();
        if (bdcdyhs == null || bdcdyhs == ''&& (djlx!=null && djlx!="" && djlx!=800&&sqlx_dm!=407&&sqlx_dm!=410)) {
            tipInfo("请选择不动产单元");
            return false;
...
//修改按钮点击事件
    $("#gdFwUpdate,#gdLqUpdate,#gdTdUpdate,#gdCqUpdate").click(function () {
        if (this.id == "gdFwUpdate") {
            ...
            addOrUpdate(bdcdjUrl+"/gdXxLr?editFlag=true&bdclx=fw&proid=" + gdproid + "&iscp=true&qrFlag=true");
            ...
            addOrUpdate(bdcdjUrl+"/gdXxLr?editFlag=true&bdclx=td&proid=" + gdproid + "&iscp=true&tdid=" + tdid+"&qlid=" + qlid+"&qrFlag=true");
    ...
    $("#fwgjSearchBtn").click(function () {
        ...
        tableReload("fw-grid-table", fwUrl,{hhSearch: "",sfsh:"0"}, "", fwLoadComplete);
    });
...
function tdSel(proid, qlid, tdzh, qlzt,tdid) {
    //删除的原证号
...
function fwTableInit() {
    ...
    jQuery(grid_selector).jqGrid({
        url: bdcdjUrl+"/bdcJgSjgl/getGdFwJson?filterFwPpzt=${filterFwPpzt!}&sfsh=0",
        datatype: "json",
        height: $pageHight,
        jsonReader: {id: 'QLID'},
...
function tdTableInit() {
    ...
    var gridRowNum = $rownum;
    jQuery(grid_selector).jqGrid({
        url: bdcdjUrl+"/bdcJgSjgl/getGdTdJson?filterFwPpzt=${filterFwPpzt!}&sfsh=0",
        datatype: "json",
        height: $pageHight,
        jsonReader: {id: 'QLID'},
        colNames: ['序列', '权利人', '地籍号', '坐落', "土地证号", '匹配状态', '权利状态', 'QLID', '权利人', 'PROID','TDID'],
        colModel: tdColModel,
        viewrecords: true,
        rowNum: gridRowNum,
        pager: pager_selector,
