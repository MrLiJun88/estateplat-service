$(function () {
    ...
    $("#sd_search_btn").click(function () {
        ...
        if ($("#exactQueryBdcqzSd").get(0).checked) {
            exactQuery = "true";
        }
        tableReload("sd-grid-table", sdUrl, {bdcqzh: bdcqzh, zl:zl, exactQuery:exactQuery});
    });
...
function scfj(sdid) {
    if (sdid != "") {
        var url =platformUrl+"/fc.action?readOnly=true&from=task&proid=" + sdid;
...
function sdInitTable() {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        height: 'auto',
        jsonReader: {id: 'SDID'},
        colNames: ['序列', '权证号', /*'不动产单元',*/ '坐落', '限制原因', '限制状态', '锁定人', '解锁人', '锁定时间','解锁时间','操作','PROID','SDID','LY','QLID'/*,'修改'*/],
        colModel: [
            {
                name: 'XL',
                index: '',
                ...
                }},
            {
                ...
                formatter: function (cellvalue, options, rowObject) {
                    return '<div style="margin-left:8px;">' +
                        ' <div title="上传附件" style="float:left;cursor:pointer;" class="ui-pg-div ui-inline-edit" id="" onclick="scfj(\'' + rowObject.SDID + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="ace-icon fa fa-cog  bigger-120 blue"></span></div>' +
                        '</div>'
                }
            },
