...
$mulDataFw = new Array();
$mulDataTd = new Array();
$mulDataFwTd = new Array();
$mulRowidFw = new Array();
$mulRowidTd = new Array();
$mulRowidFwTd = new Array();
...
tdColModel = [
    ...
    //        return '<span style="font-family: cursive;"> ' + rowObject.ROWNUM_ + '. </span><input type="radio" name="tdXl"  onclick="tdSel(\'' + rowObject.PROID + '\',\'' + rowObject.QLID + '\',\'' + rowObject.TDZH + '\',\'' + rowObject.QLZT + '\',\'' + rowObject.TDID + '\')"/>'
    //    }
    //},
    {name: 'XL', index: 'XL', width: '10%', sortable: false},
...
dyhColModel = [
    {
        ...
        return '<span style="font-family: cursive;"> ' + rowObject.ROWNUM_ + '. </span><input type="checkbox" name="dyhXl" value="' + rowObject.BDCDYH + '" djId="' + rowObject.ID + '" bdcdyfwlx="' + rowObject.BDCDYFWLX + '"/>'
    }
    },
    ...
    {name: 'QLR', index: '', width: '15%', sortable: false},
    {
        ...
        sortable: false,
        formatter: function (cellvalue, options, rowObject) {
            return '<span class="showBdcdy" onclick="showBdcdyxx(\'' + rowObject.BDCDYH + '\',\'' + rowObject.ID + '\')"> '
                + cellvalue + '</span>'
        }
    },
...
function showBdcdyxx(bdcdyh, id) {
    $.ajax({
        type: "GET",
        url: bdcdjUrl + "/bdcJgSjgl/getBdcdyidOrFwid",
        data: {bdcdyh: bdcdyh, fwDcbIndex: id},
        success: function (result) {
            var bdcdyid = result != '' && result != undefined ? result : id;
            showBdcdyxxFr(bdcdyid)
        },
        error: function (data) {
        }
    });
...
function showBdcdyxxFr(bdcdyid) {
    var url = reportUrl + "/ReportServer?reportlet=query%2Fbdc_bdcdy.cpt&op=write&bdcdyid=" + bdcdyid;
    openWin(url);
}
...
fwtdSelLoadComplete = function () {
    debugger;
    $mulDataFwTd.splice(0, $mulDataFwTd.length);
    $mulRowidFwTd.splice(0, $mulRowidFwTd.length);
    $("#gdFwTdMulXx").html("<span>已选择(" + $mulRowidFwTd.length + ")</span>");
    ...
    var jqData = $(grid_selector).jqGrid("getRowData");
    var selPpzt = $("#selPpzt").val();
    $.each(jqData, function (index, data) {
        getDyYgCfStatus($(grid_selector), data.TDID, "TD");
        if(mulFczTdzPp != "true"){
            if(!reload){
                debugger;
                $("#" + data.TDID).find("input[name='fwtdXl']").prop("checked", 'checked');
                getFwTd(data.TDID);
            }
        }else{
            //吴江多对多匹配不需要勾选房产证
            if(selPpzt != "0" || selPpzt == ""){
                if(!reload){
                    debugger;
                    $("#" + data.TDID).find("input[name='fwtdXl']").prop("checked", 'checked');
                    getFwTd(data.TDID);
...
$(function () {
    ...
    $("#fw_search").click(function () {
        if(showOptimize == "true"){
            var qlr = $("#fw_search_qlrmc").val();
            var zl = $("#fw_search_zl").val();
            var fczh = $("#fw_search_fczh").val();
            var fuzzyQuery = $("#fuzzyQueryfw").get(0).checked ? "true" : "false";
    ...
    $("#td_search").click(function () {
        if (showOptimize == "true"){
            var djhSearch= $("#td_search_djh").val();
            var tdZlSearch = $("#td_search_zl").val();
            var tdzhSearch = $("#td_search_tdzh").val();
            var fuzzyQueryTd = $("#fuzzyQueryTd").get(0).checked ? "true" : "false";
            if ((djhSearch == "" || djhSearch == null || djhSearch == undefined)
                &&(tdZlSearch == "" || tdZlSearch == null || tdZlSearch == undefined)
                &&(tdzhSearch == "" || tdzhSearch == null || tdzhSearch == undefined)) {
                tipInfo("请输入坐落/土地证号/地籍号/权利人");
                return false;
            }
            var tdUrl = bdcdjUrl + "/bdcJgSjgl/getGdTdJson";
            tableReload("td-grid-table", tdUrl, {
                djhSearch: djhSearch,
                tdZlSearch:tdZlSearch,
                tdzhSearch:tdzhSearch,
                fuzzyQueryTd:fuzzyQueryTd,
                tdzh: '',
                filterFwPpzt: filterFwPpzt,
                sfsh: '1'
            }, '', tdLoadComplete);
    ...
    $("#fwTd_search").click(function () {
        ...
        $bdclx = "TDFW";
        tableReload("fwTd-grid-table", tdUrl, {hhSearch: hhSearch, tdzh: '', tdid: '', qlid: ''}, '', fwtdLoadComplete);
    })
...
//                }
    ...
    $("#gdFwTdMulXx").click(function () {
        var msg = "";
        if ($mulRowidFwTd.length == 0) {
            msg = "请选择土地证";
            tipInfo(msg);
            return;
        }
        var qlids = '';
        var qlidArr = new Array();
        for (var i = 0; i < $mulRowidFwTd.length; i++) {
            qlidArr.push($mulRowidFwTd[i])
        }
        var qlids = qlidArr.join(",");
        url = bdcdjUrl + "/bdcJgSjgl/qurrySelectGdTd?qlids=" + qlids;
        showIndexModel(url, "列表详情", "1000", "650", false);
...
//                }
                ...
                if (wfids != null && wfids != "" && wfids.indexOf(sqlxdm) < 0) {
                    ...
                    if ($("#fw").hasClass("active") && (fwtdids == null || fwtdids == '') && matchTdzh == "true") {
                        ...
                        showConfirmDialog("提示信息", msg, "beforedyhPic", "'" + gdproid + "','" + bdcdyh + "','" + tdzh + "','" + fwid + "','" + tdid + "','" + ppzt + "','" + djId + "','" + qlid + "','" + lqid + "','" + cqid + "'", "", "");
                    } else {
                        beforedyhPic(gdproid, bdcdyh, tdzh, fwid, tdid, ppzt, djId, qlid, lqid, cqid);
                    }
                } else {
                    beforedyhPic(gdproid, bdcdyh, tdzh, fwid, tdid, ppzt, djId, qlid, lqid, cqid);
                }
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == '0' || data == '') {
                            ...
                            var checkBool = checkThisQlidAvailable(qlid,"fw",bdcdyh);
                            if (checkBool == "false" && checkBool != null) {
                                //showConfirmDialog("提示信息", "该不动产单元已创建项目，是否解除匹配", "disPic", "'" + gdproid + "','" + tdid + "','" + lqid + "','" + cqid + "','" + qlid + "'", "", "");
                                tipInfo("该房产证已创建项目！");
                            } else {
...
//修改按钮点击事件
    ...
    $("#hjhtdzsearch").click(function () {
        window.open(etlUrl+"/tddj");
    })
    //项目表高级查询的搜索按钮事件
    ...
    function checkThisQlidAvailable(qlid,type,bdcdyh) {
        var available = '';
        $.ajax({
            url: bdcdjUrl + '/bdcJgSjgl/checkThisQlidAvailable?qlid=' + qlid + '&type=' + type + '&bdcdyh='+ bdcdyh,
            datatype: 'GET',
            async: false,
            success: function (data) {
                available = data;
    ...
    $("#lockFw,#gdTdLocked").click(function () {
        ...
        $.ajax({
            ...
            success: function (jsonData) {
                if (jsonData == false) {
                    $.Prompt("该过渡数据已被锁定，请先解锁再进行锁定！",1500);
                    return false;
                } else {
                    $("#gdXzyyPop").show();
                    $(window).trigger('resize.chosen');
...
//            $(".chosen-select").trigger('chosen:updated');
    ...
    $("#gdFwTdclean").click(function () {
        debugger;
        reload = true;
        $mulDataFwTd.splice(0, $mulDataFwTd.length);
        $mulRowidFwTd.splice(0, $mulRowidFwTd.length);
        $("#gdFwTdMulXx").html("<span>已选择(" + $mulRowidFwTd.length + ")</span>");
        $("#fwTd-grid-table").trigger("reloadGrid");
        tipInfo("清空成功");
    });
    ...
    $("#gdfwMergeData,#gdtdMergeData").click(function () {
        var qlidArr = new Array();
        ...
        if(qlidArr.length > 1) {
            var qlids = qlidArr.join(",");
            $.ajax({
                url: bdcdjUrl + '/gdDataMerge/checkGdDataMerge',
                type: 'post',
                datatype: 'json',
                data: {qlids: qlids,bdclx: $bdclx},
                success: function (data) {
                    if (data.isMerge == "no") {
                        tipInfo(data.msg);
                    } else if (data.isMerge == "yes") {
                        var msg = data.msg;
                        showConfirmDialog("提示信息", msg, "mergeGdData", "'" + qlids + "'", "", "");
                    }
                },
                error: function (data) {
...
function mergeGdData(qlids) {
    $.ajax({
        url: bdcdjUrl + '/gdDataMerge/mergeGdData',
        type: 'post',
        datatype: 'json',
        data: {qlids: qlids, bdclx: $bdclx},
        success: function (data) {
                            ...
                            if (fwtdids != null && fwtdids != "") {
                                ...
                                if (result.length > 1) {
                                    $.each(result, function () {
                                        if (this["tdqlid"])
                                            tdqlid.push(this["tdqlid"]);
                                    });
                                } else {
                                    tdqlid.push(result[0].tdqlid);
                                }
                                selFwTdByFw("", "", tdqlid.join(","));
                                $("#tdids").val("");
                                $("#tdid").val(tdqlid.join(","));
                            } else {
                                $("#fwTd_search_qlr").next().hide();
                                $("#fwTd_search_qlr").val(qlrmc);
...
//多个房产证匹配不动产单元号情况
var beforedyhPic = function (gdproid, bdcdyh, tdzh, fwid, tdid, ppzt, djId, qlid, lqid, cqid) {
    if ($bdclx == 'TDFW' && ($mulDataFw.length > 1 || tdid && tdid.indexOf(",") > -1)) {
        var msg = null;
        if ($mulDataFw.length > 1)
            msg = "是否选择多个房屋权利进行匹配？";
        else if (tdid.indexOf(",") > -1)
            msg = "该房屋匹配多个土地，查看是否跨宗？";
        showConfirmDialog("提示信息", msg, "kzDyhPic", "'" + bdcdyh + "','" + tdid + "','" + djId + "'", "", "")
    } else {
...
//跨宗匹配不动产单元号
var kzDyhPic = function (bdcdyh, tdid, djId) {
    var paramArray = [];
    var fwqlids = [];
    var fwids = [];
    $.each($mulDataFw, function () {
        var param = {};
        var proid = this["PROID"];
        var fwqlid = this["QLID"];
        var fwid = getFwid(fwqlid);
        param["proid"] = proid;
        param["qlid"] = fwqlid;
        param["fwid"] = fwid;
    ...
    $.ajax({
        url: bdcdjUrl + "/bdcJgSjgl/checkKzMatchData",
        type: "post",
        data: {bdcdyh: bdcdyh, tdid: tdid, djid: djId, qlid: fwqlids.join(","), gdid: fwids.join(",")},
        success: function (data) {
            if ($.isEmptyObject(data)) {
                kzMatchData(bdcdyh, tdid, djId, paramArray);
            } else {
                tipInfo(data.msg);
            }
        }
...
var kzMatchData = function (bdcdyh, tdid, djId, paramArray) {
    var msg = "";
    var qlid = "";
    $.each(paramArray, function () {
        ...
        var fwid = this["fwid"];
        qlid = fwqlid;
        msg = matchDataPic(fwid, bdcdyh, tdid, djId, fwqlid, proid, "#fw-grid-table", "kz");
        if (msg && (msg == "匹配失败")) {
            return false;
        }
    });
    if (msg == "匹配成功"  && isShowPphzd == "true" && qlid != "" && qlid != null) {
        changeQtPpzt("2", $("#fw-grid-table"), qlid);
        //苏州新要求直接跳转到打印预览页面
        var url = reportUrl + "/ReportServer?reportlet=print%2Fbdc_pphzd.cpt&op=write&bdcdyh=" + bdcdyh + "&qlid=" + qlid + "&qlids=" + qlid;
        openWin(url);
    }else if(msg == "匹配成功" && isShowPphzd == "false" && qlid != "" && qlid != null){
        changeQtPpzt("2", $("#fw-grid-table"), qlid);
        tipInfo("匹配成功");
    }else{
        tipInfo(msg);
...
var dyhPic = function (gdproid, bdcdyh, tdzh, fwid, tdid, ppzt, djId, qlid, lqid, cqid) {
    ...
    if ($bdclx == "TD")
        tdid = '';
    $.ajax({
        url: bdcdjUrl+'/bdcJgSjgl/checkMatchData',
        type: 'post',
        dataType: 'json',
        ...
        success: function (data) {
            var flag = data.flag;
            if(flag == 'true'){
                matchDataPic(gdid,bdcdyh,tdid,djId,recordId,gdproid,grid_selector);
            }
            if(flag == 'false'){
                var msg = data.msg;
                tipInfo(msg);
            ...
            if(flag == 'warning'){
                var msg = data.msg;
                showConfirmDialog("提示信息", msg, "matchDataPic", "'" + gdid + "','" + bdcdyh + "','" + tdid + "','" + djId + "','" + recordId + "','" + gdproid + "','" + grid_selector + "'", "", "");
            }
...
var matchDataPic = function (gdid, bdcdyh, tdid, djId, recordId, gdproid, grid_selector, matchType) {
    var msg = "";
    $.ajax({
        ...
        colModel: [
            {
                ...
                formatter: function (cellvalue, options, rowObject) {
                    ...
                    if (bdcdyhs != null && bdcdyhs != "" && bdcdyhs.indexOf(rowObject.BDCDYH) > -1) {
                        $("#bdcdyh").val("");
                        $("#djId").val("");
                        return '<span style="font-family: cursive;"> ' + rowObject.ROWNUM_ + '. </span><input type="checkbox" name="dyhXl" checked="true"  value="' + rowObject.BDCDYH + '" djId="' + rowObject.ID + '" bdcdyfwlx="' + rowObject.BDCDYFWLX + '"/>';
                    } else if (index == 1) {
...
//                $selectArray.length = 0;
            ...
            $("#gdfwMulXx").html("<span>已选择(" + $mulRowidFw.length + ")</span>");
            $.each(jqData, function (index, data) {
                asyncGetGdFwxx(data.PROID, $(grid_selector), data.PROID, data.QLZT, data.QLID, data.FWZL);
            ...
            if (result.qlzts == null || result.qlzts == "") {
                ...
                for (var i = 0; i < qlzts.length; i++) {
                    ...
                    if (qlzts[i] == "DY") {
                        ...
                        $("#" + tdid).find("input[name='fwtdXl']").attr("checked", false);
                    } else if (qlzts[i] == "DI") {
...
function asyncGetGdFwxx(bdcid, table, rowid, qlzt, qlid, fwzl) {
    $.ajax({
        type: "GET",
        ...
        success: function (result) {
            ...
            if (result.qlzts == null || result.qlzts == "") {
                ...
                $.ajax({
                    type: "GET",
                    url: bdcdjUrl + "/queryBdcdy/getBdcdyhQlxxByBdclxAndZl?bdclx=" + $bdclx + "&zl=" + fwzl,
                    dataType: "json",
                    async: false,
                    success: function (result) {
                        if(result != null && result != "" && result != "undefined") {
                            if (result.dy!= null && result.dy != "" && result.dy != "undefined") {
                                cellVal = '';
                                cellVal += '<span class="label label-danger">抵押</span><span> </span>';
                            }
                            if (result.cf!= null && result.cf != "" && result.cf != "undefined") {
                                cellVal = '';
...
function getSdStatus(qlid, cqzh, table, proid, index, lx) {
    ...
    $.ajax({
        type: "GET",
        url: bdcdjUrl + "/bdcJgSjgl/getXzyy?cqzh=" + cqzh + "&qlid=" + qlid,
        dataType: "json",
        async: false,
        success: function (jsonData) {
...
function tdTableInit() {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        height: $pageHight,
        jsonReader: {id: 'QLID'},
        colNames: ['序列', '权利人', '锁定否', '地籍号', '坐落', "土地证号", '匹配状态', '权利状态', 'QLID', 'PROID', 'TDID', 'TDQLZT'],
        colModel: tdColModel,
        viewrecords: true,
        rowNum: gridRowNum,
        pager: pager_selector,
...
function dyhTableInit() {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        height: $pageHight,
        jsonReader: {id: 'ID'},
        colNames: ['序列', '坐落', '权利人', "不动产单元号", '不动产类型', 'ID'],
...
function fwTdTableInit() {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        colModel: [
            {
                ...
                sortable: false,
                formatter: function (cellvalue, options, rowObject) {
                    return '<span style="font-family: cursive;"> ' + rowObject.ROWNUM_ + '. </span><input type="checkbox" id="' + rowObject.TDID + '" name="fwtdXl" value="' + rowObject.TDID + '" onclick="getFwTd(\'' + rowObject.TDID + '\')"/>';
                }
            },
            {name: 'QLR', index: '', width: '23%', sortable: false},
        ...
        loadComplete: function () {
            ...
            $.each(jqData, function (index, data) {
                getDyYgCfStatus($(grid_selector), data.TDID, "TD")
                getFwTdQlr($(grid_selector),data.TDID ,data.ID);
            });
...
function getFwTdQlr(table, rowid,qlid) {
    $.ajax({
        type: "POST",
        url: bdcdjUrl + "/bdcJgSjgl/asyncGetGdqlrByLqid?lqid=" + qlid,
        dataType: "json",
        success: function (result) {
            var qlr = "";
            if (result.qlr != null || result.qlr != "") {
...
function selFwTdByFw(tdid, tdids, qlid) {
    ...
    tableReload("fwTd-grid-table", Url, data, '', fwtdSelLoadComplete);
}
...
function getBdcdyFwlx() {
    var bdcdyfwlx = $('input[name="dyhXl"]:checked')[0].attributes.bdcdyfwlx.value;
    return bdcdyfwlx;
}
...
function getFwTd(tdid) {
    var cm = $("#fwTd-grid-table").jqGrid("getRowData", tdid);
    var index = $.inArray(tdid, $mulRowidFwTd);
    var status = $("#" + tdid).find("input[name='fwtdXl']").prop("checked");
    if (status && index < 0) {
        $mulDataFwTd.push(cm);
        $mulRowidFwTd.push(tdid);
    } else if (!status && index >= 0) {
        $mulDataFwTd.remove(index);
        $mulRowidFwTd.remove(index);
    }
    var html = "<span>已选择(" + $mulRowidFwTd.length + ")</span>";
    $("#gdFwTdMulXx").html(html);
}

function getFwid(qlid) {
    var fwid = "";
    if (qlid && qlid != 'undefined') {
        $.ajax({
            url: bdcdjUrl + "/bdcJgSjgl/getFwidByQlid?qlid=" + qlid,
            type: "get",
            async: false,
            success: function (data) {
                if (data && data != 'undefied')
                    fwid = data;
            },
            error: function (data) {
                $.Prompt("查找房屋错误",1500);
...
function getFwids(qlid) {
    var fwids = "";
    if (qlid && qlid != 'undefined') {
        $.ajax({
            url: bdcdjUrl + "/bdcJgSjgl/getFwidsByQlid?qlid=" + qlid,
            type: "get",
            async: false,
            success: function (data) {
                if (data && data != 'undefied')
