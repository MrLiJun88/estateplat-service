...
fwtdSelLoadComplete = function () {
    ...
    $.each(jqData, function (index, data) {
        ...
        //if(qlzt == "注销"){
        //    $("#" + data.TDID).find("input[name='fwtdXl']").attr("checked", false);
        //}
        //getFwTd(data.TDID);
    })
...
$(function () {
    ...
    if (gdly != null && gdly != "undefined") {
        if (gdly == 'td'){
            $("#tdTab").click();
            $('#td_search_qlr').val(gdzh);
            $("#td_search_tdzh").val(gdzh);
        }
        else if (gdly == 'lq'){
            $("#lqTab").click();
        }
            ...
            $("#fwTab").click();
            $('#fw_search_qlr').val(gdzh);
            $("#fw_search_fczh").val(gdzh);
        }
    }
    /*   文字水印  */
    ...
    $("#save").click(function () {
        ...
        if (($mulDataFw.length == 1 && bdcdyhs.length == 1) || ($mulDataTd.length == 0 && $("#td") && $mulDataFw.length == 0 || ($mulDataFw.length == 1 && bdcdyhs.length == 0)) || ($mulDataTd.length == 1 && $("#td"))) {
            if (sqlxMc != null && sqlxMc != "抵押权变更登记（批量）" && sqlxMc != "" && sqlxMc.indexOf("批量") > -1) {
                $.Prompt("含有批量的申请类型需要选择2个及以上房产登记记录！",1500);
                return false;
            }
            if (mulGdfw == 'true') {
...
//                }
                ...
                $.Prompt("该项目存在多个房屋，请到多个房屋匹配页面进行匹配！",1500);
            }
        }else{
            if (fwid == '' && $("#fw").hasClass("active")) {
                $.Prompt("该项目没有房屋！",1500);
...
var kzMatchData = function (bdcdyh, tdid, djId, paramArray) {
    ...
    if (msg == "匹配成功" && isShowPphzd == "true" && qlid) {
        ...
        openWin(url);
    }else if(msg == "匹配成功" && isShowPphzd == "false" && qlid){
        changeQtPpzt("2", $("#fw-grid-table"), qlid);
        tipInfo(msg);
    }else{
        tipInfo(msg);
...
function fwTableInit() {
    ...
    jQuery(grid_selector).jqGrid({
        url: bdcdjUrl+"/bdcJgSjgl/getGdFwJson?filterFwPpzt=${filterFwPpzt!}&sfsh=1&hhSearch="+encodeURI(encodeURI(gdzh)),
        datatype: "json",
        height: $pageHight,
        jsonReader: {id: 'QLID'},
...
function tdTableInit() {
    ...
    var gridRowNum = $rownum;
    jQuery(grid_selector).jqGrid({
        url: bdcdjUrl+"/bdcJgSjgl/getGdTdJson?filterFwPpzt=${filterFwPpzt!}&sfsh=1&hhSearch="+encodeURI(encodeURI(gdzh)),
        datatype: "json",
        height: $pageHight,
        jsonReader: {id: 'QLID'},
