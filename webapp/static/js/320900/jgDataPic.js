...
tdColModel = [
    ...
    //    }
    //},
    {name: 'XL', index: 'XL', width: '5%', sortable: false},
    {name: 'SDF', index: 'SDF', width: '0%', sortable: false,hidden: true},
...
//                        if(bdcdyhs.length>1 || djlx=='800'){
    ...
    var checkMulXmFw = function(gdproids,qlids){
        var bdclx = $bdclx;
        var options = {
            url: bdcdjUrl+'/bdcJgSjgl/checkMulXmFw?wiid='+wiid,
            type: 'post',
            dataType: 'json',
            data: $("#form").serialize(),
            success: function (data) {
                if(data!=null && (data.msg=="" || data.msg==null)){
                    ...
                    if (data.hasOwnProperty("xzwh")) {
                        ...
                        if (sflw == "true") {
                            showConfirmDialog("提示信息", "" + data.msg + "，是否创建例外", "createLwGd", "'', '', '', '', '', '', '', '', '', '', '" +xzwh+"','"+examineInfo+"'", "", "");
                        } else {
                            tipInfo(data.msg);
                        }
                    } else {
...
//修改按钮点击事件
    $("#gdFwUpdate,#gdLqUpdate,#gdTdUpdate,#gdCqUpdate").click(function () {
        if (this.id == "gdFwUpdate") {
            ...
            addOrUpdate(formUrl+"/gdQlxx/gdQlxx?editFlag=true&bdclx=fw&proid=" + gdproid + "&iscp=true");
            ...
            addOrUpdate(formUrl+"/gdQlxx/gdQlxx?editFlag=true&bdclx=td&proid=" + gdproid + "&iscp=true&tdid=" + tdid+"&qlid=" + qlid);
...
function splitGdData(qlid) {
    $.ajax({
        ...
        success: function (data) {
            tipInfo(data);
            var cqzh = $("#fw-grid-table").getCell($("#qlid").val(), "FCZH");
            $("#fw_search_qlr").val(cqzh);
            $("#fw_search").click();
        },
        error: function (data) {
...
function createGlxm(proid,sqlx) {
    $.ajax({
        url: bdcdjUrl+'/bdcJgSjgl/isCancel?bdclx=' + $bdclx + "&sqlx=" + sqlx+"&wiid="+wiid,
        type: 'post',
        dataType: 'json',
        data: $("#form").serialize(),
        success: function (data) {
...
//        }
    ...
    $.ajax({
        url: bdcdjUrl+'/bdcJgSjgl/isCancel?bdclx=' + $bdclx+"&wiid="+wiid,
        type: 'post',
        dataType: 'json',
        data: $("#form").serialize(),
        success: function (data) {
...
function tdTableInit() {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        height: $pageHight,
        jsonReader: {id: 'QLID'},
        colNames: ['序列', '锁定否', '权利人', '地籍号', '坐落', "土地证号", '匹配状态', '权利状态', 'QLID', 'PROID','TDID','TDQLZT'],
        colModel: tdColModel,
        viewrecords: true,
        rowNum: gridRowNum,
        pager: pager_selector,
