...
<script type="text/javascript">
    ...
    $(function () {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            height: 'auto',
            jsonReader: {id: 'WIID'},
            colNames: ['序号', '收件编号', '证号','坐落',  '权利人', '档案号','归档人员','归档日期', 'wiid'],
            colModel: [
                {
                    name: 'XL',
                    index: '',
    ...
    function gdxxSearch() {
        ...
        slbhs = [];
        slbhs.push(slbh);
        var url = serverUrl + "/bdcGdByWiid/getGdxxPagesJson";
    ...
    function asyncGetRestOfGdxx(wiid,table,rowid) {
        ...
        $.ajax({
            type: "post",
            url: serverUrl+"/bdcGdByWiid/asyncGetRestOfGdxx?wiid=" + wiid,
    ...
    function bdcGd() {
        ...
        if (gdlx == null || gdlx == '' || gdlx == undefined){
            ...
            validateDahExist(wiids,gdlx);
        }
    }
    function validateDahExist(wiids,gdlx) {
        if (wiids != null && wiids != null){
            $.ajax({
                type:"get",
                url: serverUrl+"/bdcGdByWiid/validateDahExist?xmids=" + wiids,
                success:function (data) {
                    if(data.result != null && data.result != null){
                        if (data.result == "true"){
                            var msg = "已归档，是否重新生成档案号?";
                            showConfirmDialog("提示信息", msg, "reBdcArchive", "'"+ wiids+ "','"+ gdlx +"'","", "");
                        }else{
                            bdcArchives(wiids,gdlx);
                        }
                    }else{
                        tipInfo("验证档案号是否重复失败！");
                    }
    ...
    function bdcArchives(wiids,gdlx) {
        $.ajax({
            type:"get",
            url: serverUrl+"/bdcGdByWiid/bdcGd?wiids=" + wiids +"&gdlx="+gdlx +"&userid=" + userid,
            success:function (result) {
    ...
    function reBdcArchive(wiids,gdlx) {
        $.ajax({
            type:"get",
            url: serverUrl+"/bdcGdByWiid/reBdcArchive?xmids=" + wiids +"&gdlx="+gdlx +"&userid=" + userid,
            success:function (data) {
