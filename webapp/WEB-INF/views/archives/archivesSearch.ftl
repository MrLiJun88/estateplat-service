...
<script type="text/javascript">
    ...
    $(function () {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            height: 'auto',
            jsonReader: {id: 'WIID'},
            colNames: ['序号', '收件编号', '坐落', '不动产单元号','产权证号', '流程名称', '权利人', '档案号', '盒号', 'wiid'],
            colModel: [
                {
                    name: 'XL',
                    index: '',
            ...
            loadComplete: function () {
                ...
                $.each(jqData, function (index, data) {
                    ...
                    //翻页时保持复选框选中状态
                    if ($selectedInput.indexOf(data.WIID) != -1) {
    ...
    function gdxxSearch() {
        ...
        slbhs = [];
        slbhs.push(slbh);
        var url = serverUrl + "/bdcarchives/getGdxxPagesJson";
        tableReload("grid-table",url,{dyzmh:dyzmh,cqzh:cqzh,dah:dah,slbhs:slbh,gdlx:gdlx});
    }
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if(result.dah != null && result.dah != ''){
                    var dahCell = '<input type="text" id="gdxxdah_'+ wiid +'" name="dah" value="'+result.dah+'"style="border: none;height: 25px;width: 150px"/>';
                    table.setCell(rowid, "DAH", dahCell);
                }
                if(result.hh != null && result.hh != ''){
                    var hhCell = '<input type="text" id="gdxxhh_'+ wiid +'" name="hh" value="'+result.hh+'"style="border: none;height: 25px;width: 150px"/>';
    ...
    function bdcGd() {
        ...
        if (gdlx == null || gdlx == '' || gdlx == undefined){
            ...
            $.ajax({
                type:"get",
                url: serverUrl+"/bdcarchives/bdcGd?wiids=" + wiids +"&gdlx="+gdlx +"&userid=" + userid,
                success:function (result) {
