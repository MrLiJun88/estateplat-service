...
<script type="text/javascript">
    ...
    function printBysl() {
        var proid = "${proid!}";
        var url = "${reportUrl!}/ReportServer?reportlet=print%2Fbdc_djbgzgg.cpt&proid=" + proid+'&wiid='+wiid;
        openWin(url);
    }
</script>
    ...
    <@f.contentDiv  title="${bdcGg.ggmc!}" >
        <@f.form id="ggForm" name="ggForm">
            ...
            <div class="toparea">
                ...
                <div class="info">
                    <p>
                        根据《不动产登记暂行条例实施细则》第十七条、第八十五条的规定，拟对下列不动产登记簿的部分内容予以更正，现予公告。如有异议者请自本公告之日起十五个工作日内（${date!}之前）将异议书面材料送达我机构，逾期我机构将予以更正登记。</p>
                </div>
            ...
            <@f.table style="width:650px">
                ...
                <tbody id="bdcqk">
                    <#list resultList as value>
                        <@f.tr>
                        <@f.td colspan="1" >
                            <@f.text  id="zl_${value.bdcSpxx.spxxid!}" name="zl" value="${value.bdcSpxx.zl!}"></@f.text>
                        </@f.td>
                        <@f.td colspan="1" >
                            <@f.text  id="gznr_${bdcGg.ggid!}" name="ggnr" value="${bdcGg.ggnr!}" style="text-align:center"></@f.text>
                        </@f.td>
                        <@f.td colspan="1" >
