...
<script type="text/javascript">
    ...
    function print() {
        ...
        $.each(proidArr,function(index,item){
            //打印所有页
            p.push("{reportlet: '/print/bdc_spb.cpt', proid : '" + item + "'}");
        })
        //将参数值组成的数组转化为字符串
        var rp = p.join(",");
        //使用FineReport自带的方法cjkEncode进行转码
    ...
    function printDywqd() {
        _$.ajax({
            type: "GET",
            url: "${bdcdjUrl}/fraJax/getProidListByQllxAndWiid?wiid=${wiid!}&qllx=18",
            success: function (result) {
                var p = [];
                var printurl = "${reportUrl}/ReportServer";
                $.each(result,function(index,item){
                    //打印所有页
                    p.push("{reportlet: '/print/mul/bdc_dywqd_zhdk.cpt', proid : '" + item + "'}");
                })
                //将参数值组成的数组转化为字符串
                var rp = p.join(",");
                //使用FineReport自带的方法cjkEncode进行转码
    ...
    function printBdcqk() {
        _$.ajax({
                    ...
                    success: function (result) {
                        ...
                        $.each(result,function(index,item){
                            //打印所有页
                            p.push("{reportlet: '/print/bdc_fdcq_spb_pl_dy.cpt', proid : '" + item + "'}");
                        })
                        //将参数值组成的数组转化为字符串
                        var rp = p.join(",");
                        //使用FineReport自带的方法cjkEncode进行转码
    ...
    function printQm() {
                        var workflow_proid="${proid!}";
                        var p = [];
                        var printurl = "${reportUrl}/ReportServer";
                        p.push("{reportlet: '/print/bdc_fdcq_spb_pl_qm.cpt', workflow_proid : '" + workflow_proid+ "'}");
                        //将参数值组成的数组转化为字符串
                        var rp = p.join(",");
                        //使用FineReport自带的方法cjkEncode进行转码
                        var reportlets = FR.cjkEncode("[" + rp + "]");
            ...
            <@f.contentDiv title="审批表列表">
                <@p.listDiv>
                    <@p.toolBars>
                        ...
                        <@p.toolBar text="打印不动产情况" handler="printBdcqk" iClass="ace-icon fa fa-columns bigger-120 blue"/>
                        <@p.toolBar text="打印签名页" handler="printQm" iClass="ace-icon fa fa-columns bigger-120 blue"/>
                        <@p.toolBar text="打印抵押物清单" handler="printDywqd" iClass="ace-icon fa fa-columns bigger-120 blue"/>
                    </@p.toolBars>
