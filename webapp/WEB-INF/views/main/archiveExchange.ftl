...
$(function () {
        ...
        if (zsid != '' && zsid != 'undefined') {
            $.ajax({
                ...
                success:function (result) {
                    if (result == "成功") {
                        var Url = "${bdcdjUrl}/archiveExchange/getJjdxx?" + $("#gjSearchForm").serialize();
    ...
    jQuery(grid_selector).jqGrid({
        ...
        height:'auto',
        jsonReader:{id:'PROID'},
        colNames:["受理号", '业务登记类型', '申请人','不动产单元号', '坐落','查看档案信息' ,'proid'],
        colModel:[
            {name:'slh', index:'slh', width:'10%', sortable:false},
            {name:'ywdjlx', index:'ywdjlx', width:'15%', sortable:false},
            {name:'sqr', index:'sqr', width:'15%', sortable:false},
            {name:'bdcdyh', index:'bdcdyh', width:'23%', sortable:false},
            {name:'zl', index:'zl', width:'23%', sortable:false},
            {name:'ck', index:'', width:'10%', sortable:false, formatter:function (cellvalue, options, rowObject) {
                return '<div><div style="float:left;cursor:pointer;margin-left: 8px" class="ui-pg-div ui-inline-edit" id="" onclick="querydn(\'' + rowObject.slh + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="fa fa-search fa-lg blue"></span></div></div> '
            }
            },
...
function querydn(slh) {
    if (slh == '' || slh == null) {
        tipInfo("请选择项目");
        return false;
    }
    var url = "${archiveUrl!}/gateway!checkArchive.action?slh=" + slh;
    openWin(url);
}
...
function serch() {
    ...
    $("#qlrSerch").val(dcxc);
    var Url = "${bdcdjUrl}/archiveExchange/getJjdxx";
    tableReload("grid-table", Url, {dcxc:dcxc, bhs:dcxc});
}
...
function qingkong() {
    $("#qlrSerch").val('');
    var Url = "${bdcdjUrl}/archiveExchange/getJjdxx";
    tableReload("grid-table", Url, {dcxc:'', bhs:'$$$'});
}
...
function plprint() {
    ...
    $.ajax({
        type:"POST",
        url:"${bdcdjUrl}/archiveExchange/creatJjd",
        data:{slh:slh},
        dataType:"json",
        success:function (result) {
...
function plprintcpt() {
    ...
    $.ajax({
        type:"POST",
        url:"${bdcdjUrl}/archiveExchange/getjjdid",
        data:{slh:slh},
        dataType:"json",
        success:function (result) {
            if (result!="") {
        ...
        <div class="simpleSearch" style="width: 1000px">
            <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn01 AdvancedButton" onclick="plprint()">生成交接单</button>
                    </td>
                    <td>
                        <button type="button" class="btn01 AdvancedButton" onclick="plprintcpt()">打印交接单</button>
                    </td>
                </tr>
            </table>
        </div>
