...
//            tableReload("fwTd-grid-table", tdUrl, {hhSearch:'',tdzh:''},'','');
        ...
        $("#fw_search").click(function () {
            ...
            var fwUrl = "${bdcdjUrl}/bdcSjgl/getGdFwJson?gdproid=${gdproid!}";
            tableReload("fw-grid-table", fwUrl, {hhSearch: hhSearch, fczh: '', dah: ''}, fwColModel, '');
        })
    ...
    var dyhPic = function (gdproid, fwid,tdid, bdcdyh, djId,tdqlid) {
        ...
        $.ajax({
            ...
            type: 'post',
            dataType: 'json',
            data: {gdproid: gdproid,gdid:recordId,bdcdyh: bdcdyh, fwid: fwid, bdclx: $bdclx, tdid: tdid,djId: djId,qlid: fwqlid,tdqlid:tdqlid},
...
//                        }
    ...
    function fwTableInit() {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            height: $pageHight,
            jsonReader: {id: 'FWID'},
            colNames: ['序列', '房屋坐落', '所在层', "总层数", '建筑面积', '匹配状态', 'PROID'],
            colModel: fwColModel,
            viewrecords: true,
            rowNum: $rownum,
            pager: pager_selector,
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result == '' || result == null) {
                    ...
                    $("#tdid").val('');
                    var tdUrl = "${bdcdjUrl}/bdcSjgl/getGdFwTdJson?fwtdz=true";
