<@com.html title="过渡数据匹配" import="ace,public">
<style>
    .filesub {
        display: block;
        width: 90px;
...
$(function () {
    ...
    $("#fwTab,#lqTab,#cqTab,#tdTab").click(function () {
        ...
        if (this.id == "dyhTab") {
            ...
            if (this.id == "fwTab") {
                ...
                fwTableInit();
                if (isLoadGrid("fw"))
                    tableReload("fw-grid-table", fwUrl, {hhSearch:'', fczh:'', dah:'', filterFwPpzt:"${filterFwPpzt!}"}, fwColModel, '');
                $("#dyhTab").click();
    ...
    $("#fw_search").click(function () {
        ...
        $("#gdproid").val("");
        var fwUrl = "${bdcdjUrl}/bdcSjgl/getGdXmFwJsonByPage";
        tableReload("fw-grid-table", fwUrl, {hhSearch:hhSearch, gdproid:'', dah:'', filterFwPpzt:'${filterFwPpzt!}'}, fwColModel, '');
    })
    ...
    $("#cjsjd").click(function () {
        ...
        $.ajax({
            url:'${bdcdjUrl}/bdcSjgl/cjSjd?gdid=' + proid + "&bdclx=" + bdclx + "&wfid=" + wfid,
            datatype:'GET',
            success:function (data) {
                ...
                $.ajax({
                    ...
                    success:function (data) {
                        if (data == '0' || data == '') {
                            ...
                            showConfirmDialog("提示信息", "是否解除匹配", "disPic", "'" + gdproid + "','" + tdid + "','" + lqid + "','" + cqid + "'", "", "");
                        } else {
                            $.Prompt("已匹配项目未删除！",1500);
                        }
                    }
                });
...
var deletefwxm = function (gdproid) {
    $.ajax({
        ...
        success:function (result) {
            ...
            $("#gdproid").val("");
            var fwUrl = "${bdcdjUrl}/bdcSjgl/getGdXmFwJsonByPage";
            tableReload("fw-grid-table", fwUrl, {gdproid:'', dah:'', filterFwPpzt:'${filterFwPpzt!}'}, fwColModel, '');
        },
        error:function (data) {
        }
    })
...
function tdSel(djh, id, qlrmc, ppzt, dyid) {
    ...
    if (tdidforTdid.split(",").length > 2) {
        $("#dyh_search_qlr").next().hide();
        $("#dyh_search_qlr").val("");
        $.ajax({
            url:'${bdcdjUrl}/bdcSjgl/getBdcdys?tdids=' + tdidforTdid,
            type:'post',
            datatype:'json',
            success:function (data) {
                selDyhByFw("", data);
...
function fwSel(bdcid, fwzl, qlrmc, slbh, ppzt,ywlx,djlx) {
    //遮罩
    $.blockUI({ message:"请稍等……" });
    //赋值
    ...
    resetFwtdids();
    var isck="";
     if(fwzl!=null && fwzl!="" && (fwzl.indexOf("车库")>-1 || fwzl.indexOf("储藏室")>-1) && ywlx!=null && ywlx!="" && ywlx=="车库余房登记"){
         isck="true";
     }
//    判断是否是多个房屋
    $.ajax({
...
//                        disableDyhTabDiv("file");
                        ...
                        picDyh(bdcid, result.fwid, qlrmc, ppzt, djlx,isck);
                        setTimeout($.unblockUI, 10);
                    }
                },
                error:function (data) {
...
//匹配不动产单元
var picDyh = function (bdcid, fwid, qlrmc, ppzt, djlx,isck) {
    //通过fczh获取hs_index
    var bdcdyhs = getBdcdyhs();
    var fwtdids = getFwtdids();
...
var checkXm = function (gdproid, bdcdyh, tdzh, fwid, tdid, dyid, ygid, cfid, yyid, ppzt, djId) {
    ...
    $.blockUI({ message:"请稍等……" });
    $.ajax({
        url:'${bdcdjUrl}/bdcSjgl/isCancel?lx=' + $bdclx,
        type:'post',
        dataType:'json',
        data:$("#form").serialize(),
        success:function (data) {
...
var disPic = function (gdproid, tdid, lqid, cqid) {
    var gdid = '';
    var gdtab = '';
    if ($("#fw").hasClass("active")) {
        gdid = gdproid;
    ...
    $.ajax({
        ...
        type:'post',
        datatype:'json',
        data:{gdid:gdid, gdproid:gdproid, tdid:tdid, lqid:lqid, cqid:cqid, gdtab:gdtab},
        success:function (data) {
            if (data == "fail") {
                alert("删除失败！");
            } else if (data == "error") {
...
var createXm = function (gdproid, bdcdyh, tdzh, fwid, tdid, dyid, ygid, cfid, yyid, ppzt, gdid, grid_selector, djId) {
    ...
    var options = {
        ...
        success:function (matchData) {
            ...
            $.ajax({
                url:'${bdcdjUrl}/bdcSjgl/creatCsdj?lx=' + $bdclx + "&tdids=" + tdidforTdid,
                type:'post',
                dataType:'json',
                data:$("#form").serialize(),
                success:function (data) {
...
<#--tableReload("fw-grid-table", Url, {gdproid:gdproid, dah:'', filterFwPpzt:'${filterFwPpzt!}'}, fwColModel, '');-->
...
function tdTableInit() {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        height:$pageHight,
        jsonReader:{id:'TDID'},
        colNames:['序列', '原地籍号', '地籍号', '土地证号', "坐落", '匹配状态', 'ID', 'DYID'],
        colModel:[
            {name:'XL', index:'', width:'10%', sortable:false, formatter:function (cellvalue, options, rowObject) {
                return '<span style="font-family: cursive;"> ' + rowObject.ROWNUM_ + '. </span><input type="hidden" name="tdidTemp" value="' + rowObject.TDID + '" ><input type="checkbox" name="tdXl"  onclick="tdSel(\'' + rowObject.DJH + '\',\'' + rowObject.TDID + '\',\'' + rowObject.ZL + '\',\'' + rowObject.PPZT + '\',\'' + rowObject.DYID + '\')"/>'
            }
            },
            {name:'DJH', index:'DJH', width:'15%', sortable:false},
...
function selFwTdByFw(tdid, tdids) {
    ...
    if (tdids != null && tdids != "")
        tdids = tdids.join(",");
    var data = {hhSearch:'', tdid:tdid, tdids:tdids};
...
function yzGlxmxx(proid) {
    ...
    var pd = false;
    $.ajax({
        url:"${bdcdjUrl}/bdcSjgl/yzGlxmxx?proid=" + proid + "&qlr=" + qlr + "&zl=" + zl,
        type:"GET",
        async:false,
        success:function (data) {
            pd = data;
...
<div class="row">
    ...
    <div class="col-xs-3">
        ...
        <#if "${sysVersion!}"=="hm">
            <button type="button" class="btn btn-sm btn-primary" id="deletexmgl" style="margin-top: 5px">删除项目关联</button>
        </#if>
    </div>
</div>
