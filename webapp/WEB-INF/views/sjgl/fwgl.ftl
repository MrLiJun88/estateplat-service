<script type="text/javascript">
    //选择房产证号
    function fwSel(fczh,id){
        if(fczh && fczh!='undefined'){
            $("#fczh").val(fczh);
        }else{
            $("#fczh").val("");
        }
        ...
        jQuery(grid_selector).jqGrid({
            ...
            colModel:[
                {name:'XL',index:'', width:'9%',sortable:false,formatter:function(cellvalue, options, rowObject){
                    return '<span style="font-family: cursive;"> '+rowObject.ROWNUM_+'. </span><input type="radio" name="fwXl" onclick="fwSel(\''+rowObject.FCZH+'\',\''+rowObject.FWID+'\')"/>'
                }
                },
