...
$(function () {
    ...
    $("#search").click(function () {
		var sqr = $.trim($("#search_sqr").val());
        var fwzl =$.trim($("#search_fwzl").val());
        var sfzh =$.trim($("#search_sfzh").val());
        var cqzh =$.trim($("#search_cqzh").val());
        if (sqr == null || sqr =='') {
			tipInfo("申请人不能为空！");
			return false;
		}
		var Url = "${bdcdjUrl}/queryfwxx/getFwxxByPagesJson";
       	tableReload("grid-table", Url, {fwzl:fwzl,sfzh:sfzh,cqzh:cqzh});
    });
	//open函数
...
function cjkEncode(text) {                                                                            
    ...
    jQuery(grid_selector).jqGrid({
        ...
        colNames:["权利人",'不动产单元号','房屋坐落','结构','用途','建筑面积','分摊面积','bdcdyid','查看'],
        colModel:[
			{name:'QLR', index:'QLR', width:'15%', sortable:false},
			{name:'BDCDYH', index:'BDCDYH', width:'23%', sortable:false},
			{name:'ZL', index:'ZL', width:'20%', sortable:false},
			{name:'FWJG', index:'FWJG', width:'13%', sortable:false},
			{name:'YT', index:'YT', width:'5%', sortable:false},
			{name:'JZMJ',inde:'JZMJ',width:'5%',sortable:false},
			{name:'FTJZMJ',inde:'FTJZMJ',width:'5%',sortable:false},
			...
			{name:'ck', index:'', width:'5%', sortable:true, formatter:function (cellvalue, options, rowObject) {
                    return '<div style="margin-left:5px;"> <div title="查看" style="float:left;cursor:pointer;" class="ui-pg-div ui-inline-edit" id="" onclick="openDetails(\'' + rowObject.BDCDYID + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="ace-icon fa fa-search  bigger-80 blue"></span></div>' +
                           '</div>'
               }
            }
...
function openDetails(bdcdyid){
		var sqr = encodeURI(encodeURI($("#search_sqr").val()));
		var sfzh = $("#search_sfzh").val();
    	var url='${reportUrl!}/ReportServer?reportlet=edit%2Fbdc_fwxxcxjg.cpt&op=write&bdcdyid='+bdcdyid+'&sqr='+sqr+'&sfzh='+sfzh;
        openWin(url);
}
	...
	<div class="page-content">
		<div class="simpleSearch">
			<table cellpadding="0" cellspacing="0" border="0">
				<tr>
					<td style="border:0px">
						<label class="ace-icon " style="width:60px">申请人：</label>
					</td>
					<td>
					...
					<td style="border: 0px">
						<label class="ace-icon " style="width:70px">房屋坐落：</label>
					</td>
					<td>
						<input type="text" class="input-icon" id="search_fwzl" style="height: 35px;border:0px solid;line-height:35px"
                    ...
                    <td style="border: 0px">&nbsp;&nbsp;&nbsp;</td>
                    <td style="border: 0px">
                        <label class="ace-icon " style="width:100px">不动产权证号：</label>
                    </td>
                    <td>
                        <input type="text" class="input-icon" id="search_cqzh" style="height: 35px;width:240px;border:0px solid;line-height:35px"
                               data-watermark="请输入不动产权证号">
                    </td>
					<td style="border: 0px">&nbsp;&nbsp;&nbsp;</td>
					<td class="Search">
