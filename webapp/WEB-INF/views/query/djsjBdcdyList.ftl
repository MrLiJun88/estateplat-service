...
<script type="text/javascript">
    ...
    function ywsjInitTable() {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            height: 'auto',
            jsonReader: {id: 'ZSID'},
            colNames: ['不动产单元号', '不动产权证号', '坐落', '权利人', '不动产类型','PROID','ZSID'],
            colModel: [
                {
                    name: 'BDCDYH',
                    index: '',
                    ...
                    if (data.result) {
                        ...
                        if (data.checkModel == "ALERT"){
                            if(item.checkCode=='199'){
                                ...
                                $("#csdjAlertInfo").append('<div class="alert alert-danger"><span style="cursor:pointer" class="label label-sm label-primary arrowed arrowed-right pull-right" onclick="openProjectInfo(\'' + bdcdyh + '\')" >查看</span>' + data.msg + '</div>');
                            }
                        } else if (data.checkModel == "CONFIRM") {
                            $("#csdjConfirmInfo").append('<div class="alert alert-warning"><span style="cursor:pointer" class="pull-right label label-sm label-primary arrowed-in"  onclick="createXm(\'' + gdproid + '\',\'' + bdcdyh + '\',\'' + qlid + '\',\'' + bdclx + '\',\'' + djid + '\')">忽略</span><span style="cursor:pointer" class="label label-sm label-primary arrowed-right pull-right" onclick="openProjectInfo(\'' + bdcdyh + '\')">查看</span>' + data.msg + '</div>');
                        }
                        $("#tipPop").show();
                        $("#modal-backdrop").show();
                    } else {
                ...
                if (data == '成功') {
                    ...
                    if(bdclx=="TDFW") {
                        ...
                        if (check == "true") {
                            ...
                            showConfirmDialog("提示信息", msg, "fsssEditXm", "'" + id + "'", "", "");
                        }else{
                            window.parent.hideModel();
                            window.parent.resourceRefresh();
                        }
