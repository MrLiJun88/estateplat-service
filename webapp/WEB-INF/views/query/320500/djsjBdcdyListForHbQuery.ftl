...
//            var fzjssj = $("#fzjssj").val();
            ...
            var zsly = $("#zslySelect").val();
            if(dcxc == "" || dcxc == null ||dcxc == undefined) {
                tipInfo("请输入不动产权证号/房产证号/权利人/坐落/不动产单元号");
            }else{
                var ywsjUrl = "${bdcdjUrl}/selectBdcdy/getBdczsListAndGdfczOrTdzListByPage?zdtzm=${zdtzm!}&dyfs=${dyfs!}&qllx=${yqllxdm!}&bdclxdm=${bdclxdm!}&qlxzdm=${qlxzdm!}&proid=${proid!}&ysqlxdm=${ysqlxdm!}&zsly=" +zsly;
                tableReload("ywsj-grid-table", ywsjUrl, {dcxc: $("#ywsj_search").val()});
            }
        })
        ...
        $("#gdtdsj_search_btn").click(function () {
            ...
            if(dcxc == "" || dcxc == null ||dcxc == undefined) {
                ...
                var gdtdsjUrl = "${bdcdjUrl}/selectBdcdy/getBdczsListAndGdfczOrTdzListByPage?zdtzm=${zdtzm!}&dyfs=${dyfs!}&qllx=${yqllxdm!}&bdclxdm=${bdclxdm!}&qlxzdm=${qlxzdm!}&proid=${proid!}&ysqlxdm=${ysqlxdm!}&isTd=true&zsly=" +zsly;
                tableReload("gdtdsj-grid-table", gdtdsjUrl, {dcxc: $("#gdtdsj_search").val()});
            }
        ...
        $("#ywsjGjSearchBtn").click(function () {
            var ywsjUrl = "${bdcdjUrl}/selectBdcdy/getBdczsListAndGdfczOrTdzListByPage?zdtzm=${zdtzm!}&dyfs=${dyfs!}&qllx=${yqllxdm!}&qlxzdm=${qlxzdm!}&bdclxdm=${bdclxdm!}&proid=${proid}&ysqlxdm=${ysqlxdm!}&zsly=bdc&" + $("#ywsjSearchForm").serialize();
        ...
        $("#tdsjGjSearchBtn").click(function () {
            var gdtdsjUrl = "${bdcdjUrl}/selectBdcdy/getBdczsListAndGdfczOrTdzListByPage?proid=" + $("#proid").val() + "&" + $("#tdsjSearchForm").serialize();
        ...
        $("#cfsjGjSearchBtn").click(function () {
            var gdcfsjUrl = "${bdcdjUrl}/selectBdcdy/getGdcfListByPage?&" + $("#cfsjSearchForm").serialize();
    ...
    function ywsjInitTable() {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            height: 'auto',
            jsonReader: {id: 'ID'},
            colNames: ['不动产单元号', '不动产权证号', '坐落', '权利人', '不动产类型','项目来源', 'DJID','BDCID','ID','GDDYH','GDZL','GDBDCLX','ZSID'],
            colModel: [
                {
                    name: 'BDCDYH',
                    index: '',
                ...
                {name: 'XMLY', index: 'XMLY', width: '6%', sortable: false},
                {name: 'DJID', index: 'DJID', width: '6%', sortable: false, hidden: true},
                {name: 'BDCID', index: 'BDCID', width: '6%', sortable: false, hidden: true},
                {name: 'ID', index: 'ID', width: '6%', sortable: false, hidden: true},
                {name: 'GDDYH', index: 'BDCDYH', width: '6%', sortable: false, hidden: true},
                {name: 'GDZL', index: 'FWZL', width: '6%', sortable: false, hidden: true},
                {name: 'GDBDCLX', index: 'BDCLX', width: '6%', sortable: false, hidden: true},
                {name: 'ZSID', index: 'ZSID', width: '6%', sortable: false, hidden: true}
            ],
            viewrecords: true,
            rowNum: 7,
            rowList: [7, 15, 20],
            ...
            loadComplete: function () {
                ...
                var url = $(grid_selector).getGridParam("url");
                $.each(jqData, function (index, data) {
                    getDetailInfo(data.PROID,data.BDCQZH,$(grid_selector),data.ID,data.GDDYH,data.GDZL,data.GDQLR,data.GDBDCLX,data.XMLY);
                    getXmly(data.XMLY,$(grid_selector),data.ID);
                });
    ...
    function getDetailInfo(proid,bdcqzh, table, rowid,gddyh,gdzl,gdqlr,gdbdclx,xmly){
        if(xmly == '1'){
            $.ajax({
                type: "GET",
                url: "${bdcdjUrl}/selectBdcdy/getdateByProid?proid=" + rowid,
                success: function (result) {
                    table.setCell(rowid, "ZL", result.zl);
                    ...
                    var bdcdyCellVal = "";
                    bdcdyCellVal = '<a href="javascript:ywsjEditXm(\'' + rowid + '\',\'' + result.bdcdyh + '\',\'' + result.bdcdyid + '\',\'' + bdcqzh + '\',\'' + result.bdclx+ '\')" title="' + result.bdcdyh + '" >' + result.bdcdyh+ "</a>";
                    table.setCell(rowid, "BDCDYH", bdcdyCellVal);
                    var bdcqzhCellVal = "";
                    bdcqzhCellVal = '<a href="javascript:ywsjEditXm(\'' + rowid + '\',\'' + result.bdcdyh + '\',\'' + result.bdcdyid + '\',\'' + bdcqzh + '\',\'' + result.bdclx+ '\')" title="' + bdcqzh + '" >' + bdcqzh+ "</a>";
                    table.setCell(rowid, "BDCQZH", bdcqzhCellVal);
                }
            });
        }else if(xmly == '2' || xmly == '3'){
            dateForAllTable(table);
        }
    }
    function getXmly(xmly,table,rowid){
        if(xmly == '1'){
            table.setCell(rowid, "XMLY", "产权");
        }else if(xmly == '2'){
            table.setCell(rowid, "XMLY", "房产");
        }else if(xmly =='3'){
            table.setCell(rowid, "XMLY", "土地");
        }
    }

    function dateForAllTable(grid_selector) {
        var jqData = $(grid_selector).jqGrid("getRowData");
        var rowIds = $(grid_selector).jqGrid('getDataIDs');
        $.each(jqData, function (index, data) {
            getdateByBdcidForAll(data.BDCID, $(grid_selector), rowIds[index], data.BDCLX, data.ZSID,data.ID,data.BDCDYH,data.PROID,data.DJID,data.BDCQZH);
        })
    }
    //获取数据
    function getdateByBdcidForAll(bdcid, table, rowid, bdclx, qlid,id,bdcdyh,gdproid,djid,fczh) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                var bdcdyCellVal = "";
                bdcdyCellVal = '<a href="javascript:gdsjEditXm(\'' + qlid + '\',\'' + bdcdyh + '\',\'' + bdclx + '\',\'' + result.zl + '\',\'' + gdproid + '\',\'' + djid + '\',\'' + bdcid+ '\')" title="' + bdcdyh + '" >' + bdcdyh + "</a>";
                table.setCell(rowid, "BDCDYH", bdcdyCellVal);
                var bdcqzhCellVal = "";
                bdcqzhCellVal = '<a href="javascript:gdsjEditXm(\'' + qlid + '\',\'' + bdcdyh + '\',\'' + bdclx + '\',\'' + result.zl + '\',\'' + gdproid + '\',\'' + djid + '\')" title="' + fczh + '" >' + fczh + "</a>";
                table.setCell(rowid, "BDCQZH", bdcqzhCellVal);
            }
        });
    }
    ...
    function gdfcInitTable() {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            height: 'auto',
            jsonReader: {id: 'ID'},
            colNames: ["不动产单元号", '房产证号', '坐落', '权利人', '不动产类型', 'GDPROID', 'DJID', 'QLID', 'bdcid','ID'],
            colModel: [
                {
                    name: 'BDCDYH',
                    index: 'BDCDYH',
...
//                dateForTable(grid_selector);
    ...
    //修改项目信息的函数
    function gdsjEditXm(qlid, bdcdyh, bdclx, zl, gdproid, djid,bdcid) {
        //遮罩
        $.blockUI({message: "请稍等……"});
    ...
    function dateForTable(grid_selector) {
        ...
        var rowIds = $(grid_selector).jqGrid('getDataIDs');
        $.each(jqData, function (index, data) {
            getdateByBdcid(data.BDCID, $(grid_selector), rowIds[index], data.BDCLX, data.QLID,data.ID);
        })
    }
    //获取数据
    function getdateByBdcid(bdcid, table, rowid, bdclx, qlid,id) {
        if (bdcid == null || bdcid == "undefined")
            bdcid = "";
        $.ajax({
            type: "GET",
    ...
    <div class="simpleSearch">
        <table cellpadding="0" cellspacing="0" border="0">
            <tr>
                ...
                <td style="border: 0px">&nbsp;</td>
                <td>
                    <select name="zsly" id="zslySelect" class="form-control" style="width: 150px;">
                        <option value="bdcAndGd">请选择证书来源...</option>
                        <option value="bdc">产权</option>
                        <option value="gd">房产</option>
                    </select>
                </td>
                <td style="border: 0px">&nbsp;</td>
                <td>
            ...
            <tr>
                <td>
                    <input type="text" class="SSinput watermarkText" id="ywsj_search"
                           data-watermark="请输入不动产权证号/房产证号/权利人/坐落/不动产单元号">
                </td>
                <td class="Search">
...
<#--&lt;#&ndash;<input type="text" id="checkboxSelect" style="width: 200px;height: 34px;"/>&ndash;&gt;-->
<#--<table cellpadding="0" cellspacing="0" border="0">-->
<#--<tr>-->
<#--<td style="border: 0px">-->
