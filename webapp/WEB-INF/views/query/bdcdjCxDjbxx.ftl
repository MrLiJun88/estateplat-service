...
<script type="text/javascript">
    ...
    function logTableInit() {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            colNames:['不动产单元号','坐落','面积','用途','查看','DJBID','DJID','PROID','QLLX'],
            colModel:[
                ...
                {name:'CK', index:'ZDZHMJ', width:'10%', sortable:false,formatter:function (cellvalue, options, rowObject) {
                    return '<div style="margin-left:8px;">' +
                            '<div><a class="detail" href="javascript:openDetails(\'' + rowObject.PROID + '\')">查看</a></div>' +
                            '</div>'
                }},
    ...
    function openDetails(proid){
        var url ="${bdcdjUrl}/djbxxResource?proid="+proid;
        open(url);
    }
