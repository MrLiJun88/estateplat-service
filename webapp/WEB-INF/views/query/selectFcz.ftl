...
$(function () {
    ...
    $("#search").click(function () {
        ...
        if((bdcqzh == "" || bdcqzh == null || bdcqzh == undefined) && (zl == "" || zl == null || zl == undefined) && (bdcdyh == "" || bdcdyh == null || bdcdyh == undefined) && (qlr == "" || qlr == null || qlr == undefined)){
            tipInfo("请输入不动产权证号/坐落/不动产单元号/权利人!");
        }else{
            var Url = "${bdcdjUrl}/glZs/selectFcz";
            tableReload("grid-table", Url, {zl:zl,bdcdyh:bdcdyh,bdcqzh:bdcqzh,qlr:qlr,exactQuery:exactQuery},'');
        }
    })
	...
	jQuery(grid_selector).jqGrid({
		...
		height:'auto',
		jsonReader:{id:'QLID'},
		colNames: [ '不动产单元号', '不动产权证号', "坐落", '关联', 'QLID',  'PROID', 'XMLY'],
		colModel:[
			{name: 'BDCDYH', index: '', width: '15%', sortable: false},
			{name: 'BDCQZH', index: '', width: '25%', sortable: false},
		...
		loadComplete: function () {
            ...
            var jqData = $(grid_selector).jqGrid("getRowData");
            $.each(jqData, function (index, data) {
                getBdcXxByProidAndXmly(data.PROID,data.XMLY,data.QLID,$(grid_selector));
            });
		},
...
function djsjInitTable(tableKey) {
    ...
    jQuery(grid_selector).jqGrid({
        ...
        height:'auto',
        jsonReader:{id:'QLID'},
        colNames:["QLID","不动产单元号", '不动产权证号','坐落'],
        colModel:[
            ...
            {name:'ZL', index:'ZL', width:'6%', sortable:false}
        ],
        viewrecords:true,
        rowNum:7,
        rowList:[7, 15, 20],
...
function daoru(qlid,proid,xmly) {
    glfc(qlid,proid,xmly);
	/*var wiid =$("#wiid").val();
	$.blockUI({ message:"请稍等……" });
	$.ajax({
		type: "GET",
		url: "${bdcdjUrl}/glZs/glYfcz?qlids="+qlid+"&wiid="+wiid+"&proids="+proid+"&xmlys="+xmly+"&isFsss=${isFsss!}",
		dataType: "json",
...
// 异步获取坐落、不动产单元、不动产权证号
function getBdcXxByProidAndXmly(proid,xmly,qlid,table) {
    $.ajax({
        type: "GET",
        url: "${bdcdjUrl!}/glZs/getBdcXxByProidAndXmly?proid=" + proid +"&xmly="+xmly,
        success: function (result) {
            if(result!=null && result!=""){
                if(result.zl!="" && result.zl!=null && result.zl!='undefined'){
                    table.setCell(qlid, "ZL", result.zl);
                }
                if(result.bdcdyh!="" && result.bdcdyh!=null && result.bdcdyh!='undefined'){
                    table.setCell(qlid, "BDCDYH", result.bdcdyh);
                }
                if(result.bdcqzh!="" && result.bdcqzh!=null && result.bdcqzh!='undefined'){
                    table.setCell(qlid, "BDCQZH", result.bdcqzh);
                }
            }
        }
    })
...
//修改项目信息的函数
function glfc(qlids,proids,xmlys) {
    var wiid =$("#wiid").val();
    var proid =$("#proid").val();
    ...
    $.ajax({
        type: "POST",
        url: "${bdcdjUrl}/glZs/validateYfcz",
	...
	<div class="page-content">
        <form class="form advancedSearchTable" id="ywsjSearchForm">
            <div class="row">
                ...
                <div class="col-xs-1" style="width: 200px;height: 30px;">
                    <input type="text" id="bdcdyh" style="width: 200px;height: 30px;">
                </div>
        ...
        <div class="tableHeader">
            <ul>
                <li>
                    <button type="button" id="fcsjSure">
    ...
    <div class="modal-dialog tipPop-modal">
        <div class="modal-content">
            ...
            <div class="modelFooter">
                <button type="button" class="btn btn-sm btn-primary" id="tipCloseBtn">是</button>
                <button type="button" class="btn btn-sm btn-primary" id="tipCloseBtn">否</button>
            </div>
        </div>
    </div>
</div>
