...
$(function () {
	...
	$("#gjSearchBtn").click(function () {
		var Url = "${serverUrl}/bdcTdQsDj/getbdcCfPagesJsonace?" + $("#gjSearchForm").serialize();
	...
	jQuery(grid_selector).jqGrid({
		url:"${serverUrl}/bdcTdQsDj/getQlrJsonace",
		datatype:"json",
		height:'auto',
...
function donebdcFwQsDj(bdcdyid,bdcdyh) {
	openWin("${reportUrl!}/ReportServer?reportlet=com.fr.function.AnthorCpt&cpturl=/edit/bdc_tddjxx&op=write&cptName=bdc_tddjxx&bdcdyh="+bdcdyh+"&bdcdyid="+bdcdyid);
}
//xiejianan 获取不动产单元的不动产类型，权利人，坐落，权利状态信息
function getBdcdyhxx(bdcdyid,table, rowid) {
    $.ajax({
        type:"GET",
        url:"${serverUrl}/bdcTdQsDj/getBdcdyhxx?bdcdyid=" + bdcdyid,
        dataType:"json",
...
function serch() {
	...
	var Url = "${serverUrl}/bdcTdQsDj/getQlrJsonace";
