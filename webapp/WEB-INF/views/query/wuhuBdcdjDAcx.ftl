...
<script type="text/javascript">
    ...
    $(function () {
        ...
        jQuery(grid_selector).jqGrid({
            url:"${bdcdjUrl}/wuhubdcdjdacx/queryWuhuDAxxList?time="+new Date(),
            datatype:"local",
            height:'auto',
            jsonReader:{id:'BDCDYID'},
            colNames:["收件单号", '申请类型名称', '权利人', '权利人证件号','不动产权证号/预告登记证明号','不动产单元号','房屋坐落','登记时间','查封\抵押状态','查看','ly','zsid'],
            colModel:[
                {name:'SJDH', index:'SJDH', width:'5%', sortable:true},
                {name:'SQLXMC', index:'SQLXMC', width:'5%', sortable:true},
                {name:'QLRMC', index:'QLRMC', width:'10%', sortable:true},
                {name:'QLRZJH', index:'QLRZJH', width:'15%', sortable:true},
                {name:'BDCQZH', index:'BDCQZH', width:'20%', sortable:true},
                {name:'BDCDYH', index:'BDCDYH', width:'5%', sortable:true,formatter:function (cellvalue, options, rowObject) {
                	var value ='';
                	if( cellvalue != null){
                	 value= cellvalue.substr(0, 6) + " " + cellvalue.substr(6, 6) + " " + cellvalue.substr(12, 7) + " " + cellvalue.substr(19);}
                	return value;
            	}},
                {name:'FWZL', index:'FWZL', width:'10%', sortable:true},
                {name:'DJSJ', index:'DJSJ', width:'10%', sortable:true},
                //{name:'QSZT', index:'QSZT', width:'5%', sortable:false},
                {name:'DYACF', index:'DYACF', width:'5%', sortable:false},
    ...
    function openDetails(bdcdyid){
    	var url='${reportUrl!}/ReportServer?reportlet=edit%2Fbdc_djxxcxjg.cpt&op=write&bdcdyid='+encodeURI(encodeURI(bdcdyid))+'&time='+new Date();
        openWin(url);
    }
  //跳转到第二个ftl中
 <#-- function EditXm(bdcqzh,ly){
    ...
    function search_data(flag){
    	var Url = "${bdcdjUrl}/wuhubdcdjdacx/queryWuhuDAxxList?time="+new Date();
    	if(flag=='search'){
	    	yg='yes';
	    	canuse=false;
	...
	alert();
        ...
        if(!reg.test(wdqlrzjh)){
	        ...
	        if(wdqlrzjh != null && wdqlrzjh != ''  && wdqlr != null && wdqlr != ''){
		        //无档证明查询请求  返回BDCQZH
		        var Url_2 = "${bdcdjUrl}/wuhubdcdjdacx/queryWuhuDAxxList?time="+new Date();
        ...
        <div class="simpleSearch">
            <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                    ...
                    <td style="border: 0px;width:30px;">&nbsp;</td>
                     <td style="border: 0px;width:200px">
                        <button type="button"  id="printZM" style="width:120px" onclick="printZm()">打印无房证明</button>
                    </td>
                </tr>
            </table>
        </div>
