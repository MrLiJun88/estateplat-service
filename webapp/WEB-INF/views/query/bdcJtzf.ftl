...
$(function () {
    ...
    $("#savejtfw").click(function () {
        ...
        if (zsid != '' && zsid != 'undefined') {
            $.ajax({
                type:"POST",
                url:"${bdcdjUrl}/bdcJtzf/savejtfw",
                data:{zsid:zsid, czmyy:czmyy, bz:bz},
                dataType:"json",
                success:function (result) {
                    if (result == "成功") {
                        var Url = "${bdcdjUrl}/bdcJtzf/getJtzf?" + $("#gjSearchForm").serialize();
    ...
    jQuery(grid_selector).jqGrid({
        url:"${bdcdjUrl}/bdcJtzf/getJtzf",
        datatype:"json",
        height:'auto',
        jsonReader:{id:'ZSID'},
        colNames:["姓名", '证件号', '证件类型', '房屋坐落', '不动产权证号', '出证明原因', '备注', '操作', 'zsid'],
        colModel:[
            {name:'QLR', index:'QLR', width:'10%', sortable:false},
            {name:'QLRZJH', index:'QLRZJH', width:'10%', sortable:false},
            {name:'QLRSFZJZL', index:'QLRSFZJZL', width:'5%', sortable:false},
            {name:'ZL', index:'ZL', width:'15%', sortable:false},
            {name:'BDCQZH', index:'BDCQZH', width:'20%', sortable:false},
            {name:'CZMYY', index:'CZMYY', width:'12%', sortable:false},
            {name:'BZ', index:'BZ', width:'15%', sortable:false},
            {name:'CK', index:'', width:'5%', sortable:false, formatter:function (cellvalue, options, rowObject) {
                return '<div style="margin-left:8px;">' +
                        ' <div title="编辑" style="float:left;cursor:pointer;" class="ui-pg-div ui-inline-edit" id="" onclick="editjtzf(\'' + rowObject.ZSID + '\',\'' + rowObject.BDCQZH + '\',\'' + rowObject.QLR + '\',\'' + rowObject.QLRZJH + '\',\'' + rowObject.CZMYY + '\',\'' + rowObject.BZ + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="ace-icon fa fa-cog  bigger-120 blue"></span></div>' +
                        '</div>'
            }
            },
...
function serch() {
    ...
    $("#qlrSerch").val(dcxc);
    var Url = "${bdcdjUrl}/bdcJtzf/getJtzf";
    tableReload("grid-table", Url, {dcxc:dcxc, qlr:dcxc});
}
...
function qingkong() {
    $("#qlrSerch").val('');
    var Url = "${bdcdjUrl}/bdcJtzf/getJtzf";
    tableReload("grid-table", Url, {dcxc:'', qlr:'$$$'});
}
...
function plprint() {
    ...
    var qlrzjhlist = new Array();
    $.each(obj, function (index, data) {
        qlrzjhlist.push(data.QLRZJH);
    });
    var qlrzjh = "'" + qlrzjhlist.join("','") + "'";
    var url = "${reportUrl}/ReportServer?reportlet=print%2Fbdc_jtzfdjcx.cpt&op=write&ywType=server&qlrzjh=" + qlrzjh;
    openWin(url);
}
        ...
        <div class="simpleSearch">
            <table cellpadding="0" cellspacing="0" border="0">
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn01 AdvancedButton" onclick="dukaqi()">扫描</button>
                    </td>
                    <td>
                        <button type="button" class="btn01 AdvancedButton" onclick="qingkong()">清空</button>
                    </td>
                    <td>
                        <button type="button" class="btn01 AdvancedButton" onclick="plprint()">批量打印</button>
                    </td>
                </tr>
            </table>
        </div>
    ...
    <div class="modal-dialog gjSearchPop-modal">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title"><i class="ace-icon fa fa-search bigger-110"></i>编辑</h4>
            ...
            <div class="bootbox-body" style="background: #fafafa;">
                <form class="form advancedSearchTable" id="gjSearchForm">
                    <input type="hidden" id='zsid'>

                    <div class="row">
                        <div class="col-xs-2">
                            <label>不动产证号：</label>
                        </div>
                        <div class="col-xs-8" id='bdcqzh'>
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-10">
                            <select name="czmyy" id="czmyy" class="form-control" class="chosen-select "
                                    data-placeholder=" ">
                                <option></option>
                                <option value="契税征收">契税征收</option>
                                <option value="公积金贷款">公积金贷款</option>
                                <option value="商业贷款和拆迁证明">商业贷款和拆迁证明</option>
                            </select>
                        </div>
                    </div>
                    <div class="row">
            ...
            <div class="modelFooter">
                <button type="button" class="btn btn-sm btn-primary" id="savejtfw">保存</button>
            </div>
        </div>
    </div>
</div>
