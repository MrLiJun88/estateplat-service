...
$(function () {
    ...
    jQuery(grid_selector).jqGrid({
        url:"${bdcdjUrl}/bdcCfql/getbdcCfPagesJsonace",
        datatype:"json",
        height:'auto',
        jsonReader:{id:'PROID'},
        colNames:["不动产单元号", '查封文号', '查封类型', '查封机关', '轮候顺序', '权利状态',  '查看', 'proid'],
        colModel:[
            {name:'BDCDYH', index:'BDCDYH', width:'28%', sortable:false, formatter:function (cellvalue, options, rowObject) {
                if(cellvalue !=null){
            ...
            {name:'CFLX', index:'CFLX', width:'10%', sortable:false},
            {name:'CFJG', index:'CFJG', width:'15%', sortable:false},
            {name:'LHSX', index:'LHSX', width:'10%', sortable:false},
            {name:'QSZT', index:'QSZT', width:'10%', sortable:false, formatter:function (cellvalue, options, rowObject) {
                if (cellvalue == '1') {
                    cellvalue = '现势';
                    return '<span class="label label-success" style="margin-left: 20px;">' + cellvalue + '</span>&nbsp;';
                }
                else if (cellvalue == '2') {
                    cellvalue = '历史';
                    return '<span class="label label-gray" style="margin-left: 20px;">' + cellvalue + '</span>&nbsp;';
                }
            }},
//            {name:'ISJF', index:'ISJF', width:'10%', sortable:false, formatter:function (cellvalue, options, rowObject) {
//                if (cellvalue == '1') {
//                    cellvalue = '解封';
//                    return '<span class="label label-success" style="margin-left: 20px;">' + cellvalue + '</span>&nbsp;';
//                }
...
//            }},
            {name:'CK', index:'', width:'10%', sortable:false, formatter:function (cellvalue, options, rowObject) {
                return '<div style="margin-left:8px;">' +
                        ' <div title="查看" style="float:left;cursor:pointer;" class="ui-pg-div ui-inline-edit" id="" onclick="doneCfQl(\'' + rowObject.PROID + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="ace-icon fa fa-search  bigger-120 blue"></span></div>' +
                        ' <div title="证书关系" style="float:left;cursor:pointer; margin-left: 8px;" class="ui-pg-div ui-inline-edit" id="" onclick="zsrel(\'' + rowObject.PROID + '\',\'' + rowObject.BDCDYH + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="ace-icon fa fa-sitemap"></span></div>' +
                        '</div>'
            }
            },
...
function doneCfQl(a) {
    openWin("${reportUrl!}/ReportServer?reportlet=com.fr.function.AnthorCpt&cpturl=\\edit\\bdc_cf&op=write&cptName=bdc_cf&proid=" + a + "&ywType=server&__showtoolbar__=false");
}
...
function EditXm(rowid) {
    ...
    window.open("${reportUrl!}/ReportServer?reportlet=com.fr.function.AnthorCpt&cpturl=\\edit\\bdc_cf&op=write&cptName=bdc_cf&proid=" + proid + "&ywType=server");
}
...
//        var url = "/server/bdcZsRel?proid=" + id + "&bdcdyh=" + bdcdyh;
        var url = "/workflowPlugin/index.html?url=" + "${bdcdjUrl!}" + "/bdcZsRel/getZsRelXml?proid=" + id + "&bdcdyh=" + bdcdyh;
        openWin(url);
    }
}
...
function serch() {
    ...
    var Url = "${bdcdjUrl}/bdcCfql/getbdcCfPagesJsonace";
                ...
                <tr>
                    <td>
                        <input type="text" class="SSinput watermarkText" id="search_xmmc"
                               data-watermark="请输入不动产单元号/查封文号/查封类型/查封机关">
                    </td>
                    <td class="Search">
        ...
        <div class="modal-content">
            ...
            <div class="bootbox-body" style="background: #fafafa;">
                <form class="form advancedSearchTable" id="gjSearchForm">
                    ...
                    <#list bdccfqlGjssOrderList as bdccfqlGjss>
                        <#if bdccfqlGjss == 'bdcdyh'>
                            <#if (bdccfqlGjss_index + 1) % 2 != 0>
                            <div class="row">
                            </#if>
                            <div class="col-xs-2">
                                <label>不动产单元号：</label>
