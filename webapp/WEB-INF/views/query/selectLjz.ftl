...
$(function () {
    ...
    $("#djsj_search_btn").click(function(){
        var djsjUrl = "${bdcdjUrl}/lpb/getLpbPagesJson?isFilter=" + ${isFilter!} + "&" + $("#djsjSearchForm").serialize();
    ...
    if($.cookie('lpb_cookie')!=null) {
        ...
        $("#ywsjMulXxGwc").html("");
        var s="<i  class=\"glyphicon glyphicon-shopping-cart\"></i>\n" +
                "<span>查看已选("+yxbdcdy+")</span>";
        $("#ywsjMulXxGwc").html(s);
...
function djsjInitTable(){
    ...
    jQuery(grid_selector).jqGrid({
        ...
        colNames:[ '宗地号','小区名称','坐落','操作','FW_DCB_INDEX'],
        colModel:[
            {name:'LSZD', index:'LSZD', width:'25%', sortable:false,formatter:function (cellvalue, options, rowObject) {
                    if(cellvalue!=null && cellvalue!='')
                        cell= '<a href="javascript:selectZd(\'' + rowObject.FW_DCB_INDEX + '\',\'' + rowObject.LSZD + '\',\'\',\'\')" title="'+cellvalue+'">'+cellvalue+"</a>";
                    else
                        cell='';
                    return cell;
                }},
...
function djsjSureFun(id) {
        ...
        if (id == "djsjSure"||id == "djsjMulSureBtn") {
            selectZd('','',bdcXmRelList,dcbIndexs)
        }
    }
    //打开批量信息
...
function LookLpb(id){
    var proid="${proid!}";
    location.href ="${bdcdjUrl}/lpb/lpb?dcbId="+id+"&proid="+proid+"&mulSelect=${mulSelect!}&showQl=false&type=${type!}&isNotWf=${isNotWf!}";
    //遮罩
    $.blockUI({message: "请稍等……"});
}
...
function selectZd(dcbIndex,zdzhh,bdcXmRelList,dcbIndexs){
    //遮罩
    $.blockUI({ message:"请稍等……" });
    ...
    var dcbIndexValue=dcbIndexs!=''&&dcbIndexs!='undefined'?dcbIndexs:dcbIndex;
    $.ajax({
        url:'${bdcdjUrl}/wfProject/checkLjz?proid='+proid+"&dcbIndexs="+dcbIndexValue,
        type:'post',
        dataType:'json',
        cache:false,
//        data:{proid:proid,dcbIndexs:dcbIndexs},
        success:function (data) {
            ...
            if(alertSize==0 && confirmSize==0){
                zdinit(dcbIndex,zdzhh,bdcXmRelList,dcbIndexs);
            }else if(alertSize== 0 && confirmSize>0){
                $("span[name='hlBtn']").click(function(){
                    $(this).parent().remove();
                    if($("#csdjConfirmInfo > div").size()==0){
...
//修改项目信息的函数
function zdinit(id,zdzhh,bdcXmRelList,dcbIndexs) {
    var proid="${proid!}";
    if(id!=""||dcbIndexs!=""){
        //遮罩
        //$.blockUI({ message:"请稍等……" });
        $.ajax({
            ...
            + "&bdclx=TDFW&zdzhh="+zdzhh+"&dcbIndex="+id+"&dcbIndexs="+dcbIndexs,
            success:function (data) {
                //去掉遮罩
                setTimeout($.unblockUI, 10);
                if (data == '成功') {
...
function openProjectInfo(proid){
    ...
    function djsjMulXxGwcFunc(id){
        var url="${bdcdjUrl}/lpb/selectMulLjzGwc?proid=${proid!}"+"&mulSelect=${mulSelect!}&showQl=false&type=${type!}&isNotWf=${isNotWf!}&zjgcdyFw=${zjgcdyFw!}";
        var iWidth = 1000;
        var iHeight = 600;
