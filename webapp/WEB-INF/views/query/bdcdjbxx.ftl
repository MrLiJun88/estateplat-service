...
<script src="${serverUrl}/static/js/icapturevideo.js"></script>
<script type="text/javascript">
    $(function () {
        ...
        $("#gjSearchBtn").click(function () {
            var Url = "${serverUrl}/bdcdjbxx/getbdcCfPagesJsonace?" + $("#gjSearchForm").serialize();
        ...
        jQuery(grid_selector).jqGrid({
            url:"${serverUrl}/bdcdjbxx/getQlrJsonace",
            datatype:"json",
            height:'auto',
            jsonReader:{id:'BDCDYID'},
            colNames:['不动产单元号', '权利人', '坐落', '产权证号','查看','BDCDYID'],
            colModel:[
                ...
                {name:'CK', index:'', width:'3%', sortable:false, formatter:function (cellvalue, options, rowObject) {
                    return '<div style="margin-left:8px;">' +
                            ' <div title="查看" style="float:left;cursor:pointer;" class="ui-pg-div ui-inline-edit" id="" onclick="donebdcFwQsDj(\'' + rowObject.BDCDYID + '\',\'' + rowObject.BDCDYH + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="ace-icon fa fa-search  bigger-120 blue"></span></div>'+
                            '</div>'
                }
                },
    ...
    function donebdcFwQsDj(bdcdyid,bdcdyh) {
        openWin("${reportUrl!}/ReportServer?reportlet=com.fr.function.AnthorCpt&cpturl=/edit/bdc_bdcdjbxx&op=write&cptName=bdc_bdcdjbxx&bdcdyh="+bdcdyh+"&bdcdyid="+bdcdyid);
    }
    //xiejianan 获取不动产单元的不动产类型，权利人，坐落，权利状态信息
    function getBdcdyhxx(bdcdyid,table, rowid) {
        $.ajax({
            type:"GET",
            url:"${serverUrl}/bdcdjbxx/getBdcdyhxx?bdcdyid=" + bdcdyid,
            dataType:"json",
    ...
    function serch() {
        ...
        var Url = "${serverUrl}/bdcdjbxx/getQlrJsonace";
