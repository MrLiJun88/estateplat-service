<@com.html title="土地查封查询" import="ace,public">
<style>
    .modal-dialog {
        width: 1000px;
    }
...
$(function () {
    ...
    $("#search").click(function () {
        ...
        var Url = "${bdcdjUrl}/queryTdcf/getTdcfPagesJson?" + $("#gjSearchForm").serialize();
...
$(function () {
    ...
    jQuery(grid_selector).jqGrid({
        url:"${bdcdjUrl}/queryTdcf/getTdcfPagesJson",
        datatype:"json",
        height:'auto',
        jsonReader:{id:'CFID'},
        colNames:[ '状态','登记类型', '查封单位', '查封文号', '查封开始时间', '查封结束时间', '所有权证号', '所有权人', '坐落','宗地面积', '规划用途','CFID'],
        colModel:[
            {name:'ISJF', index:'ISJF', width:'8%', sortable:false, formatter:function (cellvalue, options, rowObject) {
            ...
            {name:'CFWH', index:'CFWH', width:'30%', sortable:false},
            {name:'CFKSRQ', index:'CFKSRQ', width:'15%', sortable:false, formatter: function (cellvalue, options, rowObject) {
                if (!cellvalue) {
                    return "";
                }
                var value = cellvalue;
            ...
            {name:'CFJSRQ', index:'CFJSRQ', width:'15%', sortable:false, formatter: function (cellvalue, options, rowObject) {
                if (!cellvalue) {
                    return "";
                }
                var value = cellvalue;
        ...
        loadComplete:function () {
            ...
            $.each(jqData, function (index, data) {
                getQlrByCfid(data.CFID, $(grid_selector), data.CFID);
            })
        },
...
function getQlrByCfid(cfid,table, rowid) {
    $.ajax({
        type:"GET",
        url:"${bdcdjUrl}/queryTdcf/getQlrByCfid?cfid=" + cfid,
        dataType:"json",
