...
<script>
    ...
    $(function () {
        ...
        $('#saveInfo').click(function(){
            var array=[];
            //遍历该页的所有归档信息
            $("input[name='wiid']").each(function(i,n){
                var id = $(n).val();
                ...
                var o = new Object();
                //此处把wiid作为xmid并不准确，只是为了保存值时序列化成BdcGdxx
                o.xmid = id;
                o.ajh = ajh;
                o.mlh = mlh;
                o.ajjs = ajjs;
            ...
            if(array != "") {
                ...
                $.ajax({
                    type: "POST",
                    url: "${bdcdjUrl}/BdcGdxx/saveGdxxInfoForPl?postData=" + postData,
                    dataType: "json",
        ...
        $("#getResult").click(function () {
            ...
            if (ids.length == 0) {
              ...
              if(count==1){
                  ...
                  if (!isInArray(arrayTemp,ids[0])) {
                      $.Prompt("归档信息填写不完整，请核实！",1500);
                  } else {
                      $.blockUI({message: "请稍等……"});
                      bootbox.dialog({
                          message: "<h3><b>是否确定归档？</b></h3>",
                          title: "",
                          closeButton: false,
                          buttons: {
                              success: {
            ...
            if (ids.length > 1) {
                var isAllInArray = true;
                for(var i = 0;i < ids.length; i++){
                    if(!isInArray(arrayTemp,ids[i])){
                        isAllInArray = false;
                        break;
                    }
                }
...
//            }
             ...
             if(ids.length==1){
                 $.ajax({
                     url: '${serverUrl}/BdcGdxx/getbdcXmProidByWiid?wiid=' + ids[0],
                     type: 'get',
                     dataType: 'json',
                     success: function (data) {
                         if (data.proid&&data.proid != '') {
                             printDafp(data.proid);
                         }
                     },
                     error:function(data){
                         $.Prompt(data,1500);
        ...
        $("#print").click(function () {
            ...
            $.ajax({
                url: '${serverUrl}/BdcGdxx/getbdcXmProidByWiid?wiid=' + ids,
                type: 'get',
                dataType: 'json',
                success: function (data) {
                    if (data.proid&&data.proid != '') {
...
//            }
        ...
        //批量打印封皮
        function printAllDafp(wiids){
            var printUrl = "${serverUrl}/bdcPrint/printAllDafpByWiids?wiids=" + wiids+"&hiddeMode=false";
        ...
        $("#searchBtn").click(function () {
            ...
            if(search == "" || search == null ||search == undefined) {
                ...
                var Url = "${bdcdjUrl}/BdcGdxx/getGdxxPagesJson";
        ...
        $("#mulMlh").change(function(){
            ...
            $.ajax({
                url:'${bdcdjUrl}/BdcGdxx/getCurrentMaxAjhByMlh?mlh='+mulMlh,
                type:'GET',
                dataType:'json',
    ...
    function gd(ids){
        ...
        var options = {
            url: '${bdcdjUrl}/BdcGdxx/bdcXmGd?wiids=' + ids,
            type: 'get',
            dataType: 'json',
            success: function (result) {
                if (result != '') {
    ...
    function checkIsGd(ids){
        var options = {
            url: '${bdcdjUrl}/BdcGdxx/checkBdcXmIsGd?wiid=' + ids,
            type: 'get',
            dataType: 'json',
            success: function (result) {
                if (result != '') {
    ...
    function accessTableGrid() {
        ...
        jQuery(grid_selector).jqGrid({
            ...
            height: 'auto',
            jsonReader: {id: 'WIID'},
            colNames: ['编号', '不动产权证号', '不动产单元号', '坐落','案卷号','目录号','案卷件数','案卷页数', '归档人', '归档日期', '归档状态','打印档案封皮',<#if "${showUpFileButton!}"=="true">'上传附件',</#if> '档案ID','统编号','WIID','AWIID'],
            colModel: [
...
//                    }
                },
                {
                    ...
                    sortable: false ,
                    formatter:function(cellvalue, options, rowObject){
                        return '<div style="margin-left:20px;"><div title="编辑信息"  style="float:left;cursor:pointer; margin-left: 10px;" class="ui-pg-div ui-inline-edit" id="" onclick="showSearchDialog(\'' + rowObject.WIID + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="fa fa-cog fa-lg black"></span></div></div>'
                    }
                },
                <#if "${showUpFileButton!}"="true">
                                {
                                    ...
                                    sortable: false,
                                    formatter: function (cellvalue, options, rowObject) {
                                        return '<div style="margin-left:20px;"><div title="上传附件"  style="float:left;cursor:pointer; margin-left: 10px;" class="ui-pg-div ui-inline-edit" id="" onclick="upFileForDA(\'' + rowObject.BH + '\')" onmouseover="jQuery(this).addClass(\'ui-state-hover\');" onmouseout="jQuery(this).removeClass(\'ui-state-hover\');"><span class="fa fa-cog fa-lg black"></span></div></div>'
                                    }
                                },
                </#if>
                {name: 'DAID', index: 'DAID', width: '0%', sortable: false},
                {name: 'DBH', index: 'DBH', width: '0%', sortable: false},
//                {name: 'AWIID',index:'AWIID',width:'0%' sortable:false
//                    ,hidden:true,
//                    formatter:function(cellvalue, options, rowObject){
//                        return '<input type="hidden" name="wiid" value="'+cellvalue+'"style="height: 30px;width: 150px"/>';
//                    }
//                },
...
//                    }
                ...
                {name: 'AWIID',index:'AWIID',width:'0%',sortable:false,hidden:true,
                    formatter:function(cellvalue, options, rowObject){
                        return '<input type="hidden" name="wiid" value="'+cellvalue+'"style="height: 30px;width: 150px"/>';
                    }
                }
            ...
            loadComplete: function () {
                ...
                $.each(jqData, function (index, data) {
                    loadDataForTable(data.WIID,$(grid_selector),data.WIID);
                    loadGdxxForTable(data.WIID,$(grid_selector),data.WIID);
                });
...
//                }
    ...
    function loadDataForTable(wiid,table,rowid){
        $.ajax({
            type: "GET",
            url: "${bdcdjUrl}/BdcGdxx/getBdcqxxByWiid?wiid=" +wiid,
            success: function (result) {
    ...
    function loadGdxxForTable(wiid,table,rowid){
        var cellValue = '<span class="label label-warning">未归档</span>';
        $.ajax({
            type: "GET",
            url: "${bdcdjUrl}/BdcGdxx/getBdcGdxxByWiid?wiid=" + wiid,
            success: function (result) {
    ...
    function upFileForDA(bh){
        $.ajax({
            type: 'post',
            url: '${bdcdjUrl}/BdcGdxx/getCqProidAndCqWiidByBh',
            data: {bh: bh},
            success: function (data) {
                if(data != null){
                    if(data.wiid != null && data.proid != null && data.wiid != "" && data.proid != ""){
                        var proid = data.proid;
                        var wiid = data.wiid;
                        upFile("",proid,wiid);
                    }else{
                        $.Prompt("创建收件材料失败！",1500);
                    }
                }
            },
            error: function (data) {
    ...
    function showSearchDialog(wiid){
        $.ajax({
            type:"GET",
            async:false,
            url:"${serverUrl}/BdcGdxx/getCurrentGdxxInfo?wiid="+wiid,
            dataType:"json",
            success: function (data) {
    ...
    function saveAjjsAndAjysDetail(){
        $.ajax({
            type:"POST",
            url:"${serverUrl}/BdcGdxx/saveGdxxInfo",
    ...
    function validateMaxAjh(ids){
        ...
        if(pageMlh!=null&&pageMlh!=""){
            $.ajax({
                url:'${bdcdjUrl}/BdcGdxx/getCurrentMaxAjhByMlh?mlh='+pageMlh,
                type:'GET',
                dataType:'json',
                success: function (result) {
                    if(result.ajh && result.ajh!=null){
    ...
    function saveGdxxForPl(ids){
        $.ajax({
            type:"POST",
            url:"${serverUrl}/BdcGdxx/saveMulGdxxInfo?ids=" + ids,
...
<div class="main-container">
    <div class="page-content" id="accessContent">
        ...
        <div class="tableHeader">
            <ul>
                <li>
                    <button type="button" id="getResult"  style="display: none">
            ...
            <div class="bootbox-body" style="background: #fafafa;">
                <form class="form advancedSearchTable" id="sfSearchForm">
                    ...
                    <#--<div class="row">-->
                        ...
                        <#--</div>-->
                        <#--<div class="col-xs-4">-->
                            <#--<input type="text" name="gdr" id ="gdr" class="form-control">-->
                        <#--</div>-->
                        <#--<div class="col-xs-2">-->
                            <#--<label>归档日期: </label>-->
                        <#--</div>-->
