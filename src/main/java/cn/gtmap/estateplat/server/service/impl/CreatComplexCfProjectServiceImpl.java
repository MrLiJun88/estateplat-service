...
public class CreatComplexCfProjectServiceImpl extends CreatProjectDefaultServiceImpl implements CreatProjectService {
    ...
    public List<InsertVo> initVoFromOldData(final Xmxx xmxx) {
        ...
        if (project != null && StringUtils.isNotBlank(project.getProid()) && CollectionUtils.isNotEmpty(project.getBdcXmRelList())) {
             ...
             * @description 解决问题 第二次选择时 会将第一次选择的当前项目的proid信息覆盖
             */
            ...
            if (CollectionUtils.isNotEmpty(bdcXmRelList)) {
                for (BdcXmRel bdcXmRel : bdcXmRelList) {
                    ...
                    if (StringUtils.isNotBlank(project.getSqlx()) && project.getSqlx().equals(Constants.SQLX_PLCF)) {
                        list = creatProjectCfdjServiceImpl.initVoFromOldData(project);
                    } else if (StringUtils.isNotBlank(project.getSqlx()) && (project.getSqlx().equals(Constants.SQLX_PLJF) || project.getSqlx().equals(Constants.SQLX_SFCD) || project.getSqlx().equals(Constants.SQLX_SFCD_PL))) {
                        list = creatProjectZxTxdjServiceImpl.initVoFromOldData(project);
                    }
    ...
    public void saveOrUpdateProjectData(final List<InsertVo> dataList) {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            try {
                ...
                logger.error("CreatComplexCfProjectServiceImpl.saveOrUpdateProjectData", e);
            }
