...
/**
  ...
  * @Time 2020/6/17 8:58
  * @description 验证不动产单元是否存在内部限制
  */
public class BdcdyExistNbxzValidateServiceImpl implements ProjectValidateService {
    @Autowired
                    ...
                    if (StringUtils.equals(jsonObject.getString("statuscode"), Constants.BA_FWCX_SUCCESS_CODE)) {
                        ...
                        if (CollectionUtils.isEmpty(housecodelist)) {
                            ...
                            for (int i = 0; i < housecodelist.size(); i++) {
                                ...
                                if (StringUtils.equals(housestatus, Constants.BA_FWCX_FWZT_NBXZ)) {
                                    proidList.add(project.getProid());
                                    break;
                                }
                            }
