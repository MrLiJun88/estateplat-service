 ...
 * @author <a href="mailto:liangqing@gtmap.cn;">liangqing</a>
 * @version 1.0,2016/4/29
 * @description  针对一个流程，多个不动产单元的变更登记，创建生成多本证的业务逻辑
 */

public class TurnComplexBgdjProjectServiceImpl extends TurnProjectBgdjServiceImpl {
    @Autowired
            ...
            if (CollectionUtils.isNotEmpty(bdcXmList)) {
                for (BdcXm bdcXmTemp : bdcXmList) {
                    ...
                    if(bdcQlrService.isAfgyContainGtgy(bdcQlrList)){
                        ...
                        List<BdcZs> bdcZsList = bdcZsService.creatBdcqz(bdcXmTemp, bdcQlrList,this.userid,boolPreviewZs);
                        if (CollectionUtils.isNotEmpty(bdcZsList)) {
                            list.addAll(bdcZsList);
                        }
                        //zdd 生成权利人证书关系表
