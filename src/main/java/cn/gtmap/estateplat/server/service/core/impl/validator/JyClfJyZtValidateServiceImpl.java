 ...
 * @description 江阴验证存量房网签备案状态是否已备案
 */
public class JyClfJyZtValidateServiceImpl implements ProjectValidateService {
    ...
    public Map<String, Object> validate(HashMap param) {
        ...
        if (project != null && StringUtils.isNotBlank(project.getBdcdyh())) {
            ...
            if (bdcBdcdy != null && StringUtils.isNotBlank(bdcBdcdy.getFwbm())) {
                String status = currencyService.checkJyzt(bdcBdcdy.getFwbm(), Constants.GETCLFXX_LOWERCAS);
                if ("true".equals(status)) {
                    map.put("info", project.getProid());
                }
            } else {
                logger.error("验证存量房网签备案状态失败原因：fwbm为空");
            }
        } else {
            logger.error("验证存量房网签备案状态失败原因：bdcdyh为空");
        }
        return map;
    }
