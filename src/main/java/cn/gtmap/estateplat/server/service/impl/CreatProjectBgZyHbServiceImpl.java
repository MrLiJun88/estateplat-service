 ...
 * @description 变更和转移合并
 */
public class CreatProjectBgZyHbServiceImpl extends CreatProjectDefaultServiceImpl {
    @Autowired
    ProjectService projectService;
    @Autowired
    EntityMapper entityMapper;
    @Resource(name = "creatProjectBgdjServiceImpl")
    CreatProjectBgdjServiceImpl creatProjectBgdjService;
    ...
    public List<InsertVo> initVoFromOldData(final Xmxx xmxx) {
        ...
        String sqlx2 = null;
        String sqlxdm = "";
        String bgProid = project.getProid();
        ...
        List<InsertVo> bgList = creatProjectBgdjService.initVoFromOldData(project);
        String bdcdyid = "";
        if (CollectionUtils.isNotEmpty(bgList)) {
            for (InsertVo insertVo : bgList) {
        ...
        if (CollectionUtils.isNotEmpty(zyList)) {
            for (InsertVo vo : zyList) {
                if (vo instanceof BdcXmRel) {
                    ((BdcXmRel) vo).setYproid(bgProid);
                }
                // 转移创建时间慢1,用于缮证改变qlzt排序
